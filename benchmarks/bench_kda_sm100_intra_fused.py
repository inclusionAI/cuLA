...
from cula.ops.kda.sm100.recompute_wu import recompute_w_u_from_preinverse, recompute_w_u_from_preprocessed
...
def make_equal_case(args: argparse.Namespace) -> BenchCase:
    ...
    elif args.cutedsl_variant == "flashinfer-k123-copy-fused-inv-wu":

        def cutedsl_fn(**kwargs):
            return chunk_kda_fwd_intra_sm100_equal(
                **kwargs,
                fp32_akk_workspace_only=True,
            )
    ...
    def run():
        ...
        if args.cutedsl_variant == "flashinfer-k123-copy-fused-inv-wu":
            w, u, akk = recompute_w_u_from_preinverse(out[0], k, beta, out[5])
            return (*out[:5], akk, w, u)
    ...
    def compare(cutedsl: tuple[torch.Tensor, ...], fla_with_gk: tuple[torch.Tensor, ...]):
        ...
        if args.cutedsl_variant in (
            ...
            "flashinfer-k123-copy-fused-inv-wu",
        ):
...
def main() -> None:
    ...
    if args.cutedsl_variant == "flashinfer-k123-copy-fused-inv-wu" and not args.with_recompute_wu:
        raise ValueError("the fused inverse/WU variant requires --with-recompute-wu")
