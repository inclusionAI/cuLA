...
"""
Unified benchmark: Lightning Attention prefill with decay — CuTe DSL vs FLA Triton.
...
Usage:
  ...
  python benchmarks/bench_lightning_attn_prefill.py --modes no_state h0_ht

  # Varlen only
  python benchmarks/bench_lightning_attn_prefill.py --modes varlen

  # All modes with report and plot
  python benchmarks/bench_lightning_attn_prefill.py --modes no_state h0_ht varlen --report --plot

  # Custom varlen workloads
  python benchmarks/bench_lightning_attn_prefill.py --modes varlen --num-heads 32 64 --iterations 50

  # GVA (value heads > Q/K heads)
  python benchmarks/bench_lightning_attn_prefill.py --modes no_state h0_ht varlen --num-heads 32 --num-v-heads 64
"""
...
from cula.ops.lightning.prefill import lightning_attn_fwd, lightning_attn_fwd_varlen
...
def plot_results(all_results, modes):
    ...
    out = os.path.join(
        ...
        "lightning_attn_prefill_benchmark.png",
    )
...
def generate_report(all_results, modes, args):
    ...
    path = os.path.join(
        os.path.dirname(__file__),
        "lightning_attn_prefill_benchmark.md",
    )
    with open(path, "w") as f:
        f.write("# Lightning Attention Prefill Benchmark Report\n\n")
        ...
        f.write("\n---\n*Generated by bench_lightning_attn_prefill.py*\n")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Unified benchmark: Lightning Attention prefill (CuTe DSL vs FLA)",
