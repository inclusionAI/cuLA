...
#include "gdn/sm90/utils/common.hpp"
...
struct FlatKernelTmaWarpSpecializedGdnFwd {
    ...
    struct VarlenProblemShape {
        ...
        int32_t num_seqs;
        int32_t num_heads;  // Q, K, V, O all share the same head count in GDN
        int32_t head_size;  // d
    };
    ...
    operator()(const Params& params, char* smem) {
        ...
        if (warp_group_role == WarpGroupRole::LdSt && ldst_warp_role == LdStWarpRole::LoadBetaAndAlpha) {
            if constexpr (NeedsBeta) {
        ...
        if (warp_group_role == WarpGroupRole::LdSt && ldst_warp_role == LdStWarpRole::LoadAlphaLast) {
        ...
        MainloopAlphaPipeline alpha_pipeline(storage.alpha_pipeline_storage, alpha_pipeline_params, cute::true_type{});
        ...
        if (warp_group_role == WarpGroupRole::LdSt) {
            ...
            } else if (ldst_warp_role == LdStWarpRole::LoadBetaAndAlpha) {
                if constexpr (NeedsBeta) {
                    auto work_desc = scheduler.get_next_work(params.scheduler, params.problem_size);
                    CUTE_NO_UNROLL
                    for (; work_desc.is_valid(params.scheduler);
                        ...
                        DPRINTF0_WG(
                            ...
                            work_desc.seq_len);
                        auto tile_shape = typename CollectiveMainloop::TileShape{};
                        collective_mainloop.load_beta_and_alpha(
                            params.mainloop,
                            params.problem_size,
                            tile_shape,
                            work_desc,
            ...
            } else if (ldst_warp_role == LdStWarpRole::LoadAlphaLast) {
