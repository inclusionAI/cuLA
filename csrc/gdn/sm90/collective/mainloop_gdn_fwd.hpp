...
#include "gdn/sm90/collective/common.hpp"
#include "gdn/sm90/collective/load_predicated.hpp"
#include "gdn/sm90/collective/load_tma.hpp"
#include "gdn/sm90/collective/named_barriers.hpp"
#include "gdn/sm90/collective/store_tma.hpp"
#include "gdn/sm90/kernel/options.hpp"
#include "gdn/sm90/utils/debug.hpp"
#include "gdn/sm90/utils/math_order_barrier.hpp"
#include "gdn/sm90/utils/unused.hpp"
...
namespace gdn::sm90::collective {

struct GdnNamedBarriers : FlatSharedNamedBarriers {
...
using gdn::sm90::kernel::find_option_t;
using gdn::sm90::kernel::Tag;
...
template <
    ...
    class Options>
struct FlatMainloopTmaWarpSpecializedGdnFwd {
    ...
    static_assert(NeedsAlpha && NeedsBeta, "Alpha and Beta are both used in GDN.");
    ...
    using SmemLayoutAlphaLast = decltype(make_layout(make_shape(_1{}, Int<StagesAlpha::value>{})));
    using SmemLayoutBeta = decltype(make_layout(make_shape(BlkSeqQ, Int<StagesBeta::value>{})));
    using SmemLayoutAlpha = decltype(make_layout(make_shape(BlkSeqQ, Int<StagesAlpha::value>{})));
    ...
    static constexpr int LoadAlphaBytes = size(SmemLayoutAlpha{}(_, _, _0{})) * sizeof(ElementAlpha);
    ...
    struct SharedStorage {
        ...
        alignas(alignment_for_swizzle(
            SmemLayoutAlpha{})) cute::array_aligned<ElementAlpha, cute::cosize_v<SmemLayoutAlpha>> smem_alpha;
        alignas(
    ...
    // FIXME: needs to be changed to not copy layout of Q
    float   const* ptr_Alpha; GmemStrideAlpha alpha_stride;
    ...
    load_beta_and_alpha(
        ...
        for (int blk = 0; blk < num_blocks - 1; ++blk) {
            beta_collective_load.step</*IsTail=*/false>(beta_src_dst, blk, beta_smem_pipe_write, num_blocks);
            alpha_collective_load.step</*IsTail=*/false>(alpha_src_dst, blk, alpha_smem_pipe_write, num_blocks);
        }
        beta_collective_load.step</*IsTail=*/true>(beta_src_dst, num_blocks - 1, beta_smem_pipe_write, num_blocks);
        alpha_collective_load.step</*IsTail=*/true>(alpha_src_dst, num_blocks - 1, alpha_smem_pipe_write, num_blocks);
    }
    ...
    extract_alpha_last(
        ...
        Tensor sA = make_tensor(make_smem_ptr(storage.smem_alpha.data()), SmemLayoutAlpha{});
        ...
        auto extract_loop_body = [&](int blk, auto is_final_block_) INLINE_LAMBDA {
            ...
            auto sA_curr = sA(_, alpha_smem_pipe_read.index());
            ...
            // one warp calls extract_alpha_last, so this is a guaranteed single write.
            if (thread_idx == 0) {
                sAlast_out(0) = sA_curr(B - 1);
            }
            __syncwarp();
    ...
    compute(
        ...
        Tensor sAlpha = make_tensor(make_smem_ptr(storage.smem_alpha.data()), SmemLayoutAlpha{});
        ...
        constexpr auto tiler_alpha = Shape<_64>{};
        ...
        // reduce S2R loads to quarter, 32 at a time for both loads and stores
        ...
        // tSKrV adds to tSKrSK (acc), so we set up a copy
        ...
        // kv_load here describes the 'kv cache', or the cached block state of size head_dim_k x head_dim_v
        ...
        auto kk_inv = [&](auto const& kk_smem_pipe_read) INLINE_LAMBDA {
            ...
            auto collective_inverse = CollectiveInverse(GdnNamedBarriers::StateMathWG0);
            ...
            if constexpr (NeedsBeta || !std::is_same_v<InverseType, Element>) {
                cutlass::arch::NamedBarrier::arrive_and_wait(
                    cutlass::NumThreadsPerWarpGroup, GdnNamedBarriers::StateMathWG0);
                using CopyOpS2R = SM75_U32x4_LDSM_N;
        ...
        // this method syncs with the other pipelines involved through kernel.hpp
        auto compute_loop_body = [&](int blk, auto is_first_block_, auto is_final_block_) INLINE_LAMBDA {
            ...
            auto sAlpha_curr = sAlpha(_, alpha_smem_pipe_read.index());
            ...
            auto sAlpha_slice = flat_divide(sAlpha_curr, tiler_alpha);
...
#pragma unroll
            ...
            if constexpr (is_first_block) {
                ...
                // change layout of S_t to A layout for O1
                Tensor tOrKV = make_acc_into_op<Element>(tKVrKV, typename TiledMmaO1::LayoutA_TV{});
...
#pragma unroll
            ...
            // if (blk <= 6 && thread_idx == 0 && cute::block(0)) {
            //   printf("\n");
    ...
    compute_aux_safe(
        ...
        auto qk_kk_subchunk_mma_and_store = [&](int blk) INLINE_LAMBDA {
            ...
            // O_tiled_copy uses the bf16 copy, O_tiled_copy_kk uses the InverseType copy
            ...
            auto cMqKA_subchunk = make_identity_tensor(select<0, 2>(TileShape_SubChunk{}));
            auto cMqKB_subchunk = make_identity_tensor(select<1, 2>(TileShape_SubChunk{}));
            auto tQKcMqk_subchunk = thr_mma_subchunk.partition_C(cMqk_subchunk);
            auto tQKaMqk_subchunk = thr_mma_bf16_subchunk.partition_A(cMqKA_subchunk);
            auto tQKbMqk_subchunk = thr_mma_bf16_subchunk.partition_B(cMqKB_subchunk);

            // do MMA at the granularity of 16x16x64 with two warps
            constexpr auto tiler_subchunk_qk = Shape<_16, Shape<_32, _1>>{};
            constexpr auto tiler_subchunk_alpha = Shape<_16>{};
            ...
            // (_16, _4)
            auto sAlpha_slice = flat_divide(sAlpha_curr, tiler_subchunk_alpha);
...
#pragma unroll
                ...
                broadcast_row0_operandA_to_operandB_bf16_layout(tArA_r, tArAfirst_r_j_kt, local_thread_idx);
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    tArA_r(4 * k + 3) = exp2f(tArA_r(4 * k + 3) - gf_hi);  // v0=1, v1=1
                }
...
#pragma unroll
                    ...
                    cute::transform(
                        tArA_c, tArAfirst_kt, tArA_c, [&](auto g, auto g_first) { return exp2f(g_first - g); });
                    ...
                    cute::transform(
                        tQKrKt_c_j_bf16, tArA_c, tQKrKt_c_j_float, [&](auto k, auto g) { return float(k) * g; });
        ...
        auto compute_aux_loop_body = [&](int blk, auto is_final_block_) INLINE_LAMBDA {
            ...
            cutlass::arch::NamedBarrier::arrive_and_wait(cutlass::NumThreadsPerWarpGroup, GdnNamedBarriers::AuxMath);
