...
CUTE_DEVICE void gemm_reset_zero_acc(Atom &atom, TA const &tA, TB const &tB,
                                     TC &&tC) {
...
template <typename Atom, typename TA, typename TB, typename TC>
CUTE_DEVICE void gemm_zero_acc(Atom &atom, TA const &tA, TB const &tB,
                               TC &&tC) {
...
CUTE_DEVICE constexpr auto unstageSmemLayout(Layout const &layout,
                                             Stages stages = {}) {
  return composition(
...
template <uint32_t RegCount> CUTLASS_DEVICE void warpgroup_reg_set() {
