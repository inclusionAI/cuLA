...
template <class Element, class StrideQ, class StrideK, class StrideV,
          class CollectiveMmaKV, class SmemLayoutQ, class SmemLayoutK,
          class SmemLayoutV, class TensorStorage, class PipelineQ,
          class PipelineKV, class Mask, class TileShape,
          class OrderLoadEpilogue = cute::false_type>
struct Sm100LightningFwdLoadTmaWarpspecialized {

  using TileShapeKV = typename CollectiveMmaKV::TileShape;
  ...
  using TMA_K = typename CollectiveMmaKV::Params::TMA_A;
  using TMA_V = typename CollectiveMmaKV::Params::TMA_B;
  ...
  static Params to_underlying_arguments(ProblemShape const &problem_shape,
    ...
    if constexpr (is_variable_length_v<tuple_element_t<0, ProblemShape>>) {
      ...
      if (cumulative_length_q != nullptr && cumulative_length_k != nullptr) {
        ...
        get<2>(problem_shape_qk) =
            get<2, 0>(problem_shape) + get<2, 1>(problem_shape);
        get<3>(problem_shape_qk) = get<3>(problem_shape);
      }
    } else {
      problem_shape_qk = replace<2>(
    ...
    auto problem_shape_pv = replace<1>(select<0, 2, 1, 3>(problem_shape_qk),
                                       get<2, 0>(problem_shape));

    auto params_kv = CollectiveMmaKV::to_underlying_arguments(
        problem_shape_kv,
        typename CollectiveMmaKV::Arguments{
            ptr_K,
            dK, // never used, dummy
            ptr_V,
            select<1, 0, 2>(dV),
    ...
    return Params{params_qk.tma_load_a, params_qk.tma_load_b,
  ...
  CUTLASS_DEVICE void
  load(BlkCoord const &blk_coord_in, ProblemShape const &problem_shape,
    ...
    auto [tQgQ_qdl, tQsQ] =
        tma_partition(params.tma_load_q, _0{}, make_layout(_1{}),
    ...
    auto [tKgK_kdl, tKsK] =
        tma_partition(params.tma_load_k, _0{}, make_layout(_1{}),
    ...
    auto [tVgV_dkl, tVsV] =
        tma_partition(params.tma_load_v, _0{}, make_layout(_1{}),
    ...
    // V1
    pipeline_kv.producer_acquire_bytes(pipeline_kv_producer_state,
                                       TransactionBytesLoadV);
    if (lane_predicate) {
      auto tma_barrier =
          pipeline_kv.producer_get_barrier(pipeline_kv_producer_state);
