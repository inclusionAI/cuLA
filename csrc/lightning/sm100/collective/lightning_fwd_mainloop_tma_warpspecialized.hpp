...
#include "lightning/sm100/collective/common.hpp"
#include "lightning/sm100/collective/sm100_lightning_load_tma_warpspecialized.hpp"

namespace cutlass::la::collective {

using namespace cute;

template <class Element_, class ElementKV_, class TileShape_, class StrideQ_,
          class StrideK_, class StrideV_, class Mask_,
          // shape here is QG K H
          // and referes to the two softmax warps
          // (2, 1, 1) means that they are stacked (best for large Q since it
          // loads the least K/V) (1, 2, 1) means they sit side by side (best
          // for small Q / large K)
          class ThreadShape = Shape<_2, _1, _1>,
          // Since shared memory is sufficient for FMHA, there is no need to
          // reuse shared memory.
          class OrderLoadEpilogue = cute::false_type>
struct Sm100LightningFwdMainloopTmaWarpspecialized {
  ...
  using TileShapeKV = decltype(shape_div(TileShape{}, ThreadShape{}));

  using CollectiveMmaKV = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm100, cutlass::arch::OpClassTensorOp, Element, StrideK,
      Alignment, Element, StrideV, Alignment, ElementKV, TileShapeKV,
      ClusterShape,
      cutlass::gemm::collective::StageCount<2> /* we change it later anyways*/,
      cutlass::gemm::KernelTmaWarpSpecialized1SmSm100>::CollectiveOp;

  using SmemLayoutK = decltype(unstageSmemLayout(
      typename CollectiveMmaKV::SmemLayoutB{}, Int<StageCountKV>{}));
  ...
  static_assert(TransactionBytesLoadK == TransactionBytesLoadV,
                "K and V smem layouts must be of equal size");

  using Load = Sm100FmhaLoadTmaWarpspecialized<
      Element, StrideQ, StrideK, StrideV, CollectiveMmaQK, CollectiveMmaPV,
      SmemLayoutQ, SmemLayoutK, SmemLayoutV, TensorStorage, PipelineQ,
      PipelineKV, Mask, TileShape>;
  ...
  CUTLASS_DEVICE void
  load(BlkCoord const &blk_coord, ProblemShape const &problem_shape,
  ...
  CUTLASS_DEVICE auto
  mma(BlkCoord const &blk_coord, Params const &params,
      ProblemShape const &problem_shape, TensorStorage &storage,
    ...
    Tensor sP = make_tensor(make_smem_ptr((Element *)nullptr),
  ...
  template <bool need_apply_mask, class Stage, class BlkCoord,
            class CoordTensor, class ProblemShape>
  CUTLASS_DEVICE auto
  softmax_step(float &row_max, float &row_sum, Stage stage, bool final_call,
    ...
    Tensor tStS = partition_fragment_C(typename CollectiveMmaQK::TiledMma{},
    ...
    for (int i = 0; i < size(tTMEM_LOADrS); i += 2) {
      ...
      if constexpr (size<2>(tTMEM_STORErS_x4) == _2{}) {
        if (i == size(tTMEM_LOADrS) - 6) {
          copy(tiled_tmem_store, tTMEM_STORErS_x4(_, _, 0),
  ...
  CUTLASS_DEVICE auto
  softmax(Stage stage, BlkCoord const &blk_coord, Params const &params,
    ...
    auto logical_offset =
        make_coord(get<0>(blk_coord) * get<0>(TileShape{}) +
    ...
    for (; mask_tile_count > 0; mask_tile_count -= 1) {
      softmax_step<false /* need_apply_mask */>(
          ...
          (mask_tile_count == 1) &&
              (Mask{}.get_masked_trip_count(blk_coord, TileShape{},
                                            problem_shape) == 0),
          blk_coord, cS, params, problem_shape, pipeline_s,
          pipeline_s_consumer_state, pipeline_c, pipeline_c_producer_state,
          order_s);
    ...
    for (; mask_tile_count > 0; mask_tile_count -= 1) {
      softmax_step<true /* need_apply_mask */>(
          row_max, row_sum, stage, mask_tile_count == 1, blk_coord, cS, params,
          problem_shape, pipeline_s, pipeline_s_consumer_state, pipeline_c,
  ...
  CUTLASS_DEVICE auto correction_epilogue(float scale, Stage stage,
                                          TensorO const &sO_01) {
    ...
    using TMEM_LOAD = std::conditional_t<
        kCorrectionTileSize == 32, SM100_TMEM_LOAD_32dp32b32x,
  ...
  CUTLASS_DEVICE auto
  correction(BlkCoord const &blk_coord, Params const &params,
    ...
    for (; mask_tile_count > 0; mask_tile_count -= 1) {
      ...
      float scale =
          ::exp2f(params.scale_softmax_log2 * (tTMEM_LOADVrS(kIdxOldRowMax) -
                                               tTMEM_LOADVrS(kIdxNewRowMax)));
    ...
    Tensor gLSE =
        make_tensor(make_gmem_ptr(epilogue.params.ptr_LSE),
                    select<0, 3>(problem_shape), epilogue.params.dLSE);

    correction_epilogue(params.scale_output / tTMEM_LOADVrS(kIdxFinalRowSum),
                        _0{}, sO);
    ...
    if (epilogue.params.ptr_LSE != nullptr) {
      int row_idx = get<0>(tTMEM_LOADVcS(_0{})) +
                    get<0>(TileShape{}) * get<0>(blk_coord) +
                    get<0>(TileShapeQK{});
  ...
  CUTLASS_DEVICE auto correction_empty(
      ...
      ProblemShape const &problem_shape,
      ParamsProblemShape const &params_problem_shape,
      TensorStorageEpi &shared_storage_epi, PipelineE &pipeline_epi,
      typename PipelineE::PipelineState &pipeline_epi_producer_state,
      CollectiveEpilogue &epilogue) {
