...
ChunkKDABwdIntra(
    ...
    TORCH_CHECK(
        beta.scalar_type() == at::kBFloat16 || beta.scalar_type() == at::kFloat,
        "ChunkKDABwdIntra only supports beta dtype bf16 or fp32, got ",
        beta.scalar_type());
    params.beta_is_fp32 = (beta.scalar_type() == at::kFloat) ? 1 : 0;
    params.tile_scheduler_params =
        NaiveTileScheduler::Params{tile_num, params.h, 4, params.num_sm, (int*)tile_counter.data_ptr()};
...
ChunkKDABwdWYDqkgFused(
    ...
    throw std::runtime_error(
        "chunk_kda_bwd_wy_dqkg_fused_cuda is not implemented yet. "
        "The current SM100 WY fused backward kernel is a placeholder and has been disabled to prevent invalid "
        "outputs.");
}