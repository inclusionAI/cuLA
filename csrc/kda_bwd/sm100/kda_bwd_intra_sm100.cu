...
#include "kda_bwd/sm100/gemm.h"
#include "kda_bwd/sm100/helpers.h"
#include "kda_bwd/sm100/util_func.h"
#include "kda_bwd/sm100/utils.h"
...
check_nan_array(const float* arr, int size) {
...
__device__ inline bool
check_nan_inf_array(const float* arr, int size) {
    for (int i = 0; i < size; ++i) {
        if (__isnanf(arr[i]) || __isinff(arr[i]))
            return true;
    }
    return false;
}
...
print_nan_detail(const char* name, const float* arr, int size, int idx_in_wg, int k_idx) {
    for (int i = 0; i < size; ++i) {
        if (__isnanf(arr[i])) {
        ...
        if (blockIdx.x == 0) {                                                \
            for (int _c = 0; _c < (ncols); ++_c) {                            \
                ...
                if (__isnanf(_v) || __isinff(_v)) {                           \
                    printf(                                                   \
                        "[NaN/Inf] %s(%d,%d)=%f thread=%d k_idx=%d blk=%d\n", \
                        name,                                                 \
                        (row),                                                \
                        _c,                                                   \
                        _v,                                                   \
...
constexpr int dAqk_t =
    dAqk +
    256;  // Aqk_t: [352, 368], [384, 400], [416, 432], [448, 464]  Akk_t:[368, 384], [400, 416], [432, 448], [464, 480]
constexpr int dAqk_t_02 = dAqk_t;  // Aqk_t: [352, 368], [384, 400], [416, 432], [448, 464]  Akk_t:[368, 384], [400,
                                   // 416], [432, 448], [464, 480]
constexpr int dAqk_t_13 = dAqk_t + 16 * 65536;  // lane_16
};                                              // namespace tmem_addr

enum class WarpRole { Empty = 0x0, Load = 0x1, Mma = 0x2, Compute = 0x3, Epilogue = 0x4, ComputeEpilogue = 0x5 };

static constexpr unsigned long long kWarpAssignment = 0x12'5555'5555ull;

__forceinline__ __device__ WarpRole
...
using SmemLayoutInputBF16 = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW64_Atom<bf16>{}, Shape<Int<T_TILE>, Int<K_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

template <int NUM_TILES>
using SmemLayoutMatBTF32Tranposed = decltype(coalesce(
    tile_to_shape(
        UMMA::Layout_MN_SW128_32B_Atom<tf32>{}, Shape<Int<K_TILE>, Int<SUB_T_TILE * NUM_TILES>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

using SmemLayoutInputFP32 = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW128_Atom<float>{}, Shape<Int<T_TILE>, Int<K_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// DKT exchange buffer is accessed via 128-bit vector load/store in epilogue.
// Keep row stride 16-byte aligned while adding padding.
using SmemLayoutDKTExchange = Layout<Shape<Int<T_TILE>, Int<36>>, Stride<Int<36>, _1>>;

using SmemLayoutInputTF32tmp = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW128_Atom<tf32>{}, Shape<Int<K_TILE>, Int<T_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

template <int NUM_TILES>
using SmemLayoutDA_SUB = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW64_Atom<float>{}, Shape<Int<T_TILE>, Int<16 * NUM_TILES>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

using SmemLayoutOutputBF16 =
    decltype(tile_to_shape(UMMA::Layout_K_INTER_Atom<bf16>{}, Shape<Int<T_TILE>, Int<K_TILE>>{}));

using SmemLayoutOutputTF32 =
    decltype(tile_to_shape(UMMA::Layout_K_INTER_Atom<tf32>{}, Shape<Int<T_TILE>, Int<K_TILE>>{}));

using SmemLayoutBeta = Layout<Shape<Int<K_TILE>>, Stride<_1>>;
...
struct SharedMemoryPlan {
    ...
    array_aligned<float, cosize_v<SmemLayoutDKTExchange>> b_k_exp[NUM_BUF_VALUE];
    array_aligned<float, cosize_v<SmemLayoutDKTExchange>> b_k_neg_exp[NUM_BUF_VALUE];
    // array_aligned<float, cosize_v<SmemLayoutBeta>> beta[NUM_BUF_VALUE];
    // array_aligned<float, cosize_v<SmemLayoutDB>> db;
    alignas(16) cute::uint64_t bar_load_kg_ready[NUM_BUF_VALUE], bar_load_dA_ready[NUM_BUF_A],
        bar_load_qb[NUM_BUF_VALUE], bar_load_dkg_ready[NUM_BUF_VALUE];  // load
    alignas(16) cute::uint64_t bar_kg_all_ready,
        bar_qkg_all_ready;  // CE → MMA: B-matrix data ready (phase-tracked with b_phase)
    alignas(16) cute::uint64_t bar_dq_done, bar_dkt_done;  // MMA → CE: tmem results ready (phase-tracked with b_phase)
    alignas(16) cute::uint64_t bar_dA_ready[NUM_BUF_A], bar_dAt_ready[NUM_BUF_A], bar_dA_free[NUM_BUF_A],
        bar_dAt_free[NUM_BUF_A], bar_dvalue_free[NUM_BUF_VALUE];  // epilogue
    alignas(16) cute::uint64_t bar_dA_mask_ready[NUM_BUF_A];

    alignas(16) float beta_smem[2][T_TILE];  // double-buffered per-tile beta (fp32), indexed by A_phase
...
mask_A_tensor(float* smem_ptr, int idx_in_warpgroup, int sub_len, int tmem_addr) {
    float res[TILE_SIZE];
...
__forceinline__ __device__ void
mask_At_tensor(float* smem_ptr_1, float* smem_ptr_2, int idx_in_warpgroup, int sub_len, int tmem_addr) {
    float res[32];
...
compute_epilogue_body(
    ...
    if constexpr (WG_IDX == 1) {
        db = (idx_in_warpgroup >= 64 && local_idx < sub_seq_len)
                 ? reinterpret_cast<float*>(
                       params.db_ptr)[(start_offset + tile_idx * T_TILE + local_idx) * params.h + head_idx]
                 : 0.0f;
    }
    ...
    {
        float* dA_ptr =
            idx_in_warpgroup < 64 ? shared_plan->dAqk[buf_idx_A].data() : shared_plan->dAkk[buf_idx_A].data();
    ...
    for (int k_idx = 0; k_idx < K_ITERATION; ++k_idx) {
        ...
        Tensor sKG_intra =
            make_tensor(make_smem_ptr(shared_plan->kg_all.intra[0].data()), SmemLayoutMatBTF32Tranposed<1>{});
        Tensor sQKG_intra =
            make_tensor(make_smem_ptr(shared_plan->qkg_all.intra[0].data()), SmemLayoutMatBTF32Tranposed<2>{});
        ...
        float* beta_tile = shared_plan->beta_smem[beta_buf];
        int beta_base = idx_in_warpgroup / 8;
        float2 beta1 = make_float2(0.0f, 0.0f);
        float2 beta2 = make_float2(0.0f, 0.0f);
        float2 beta3 = make_float2(0.0f, 0.0f);
        if (beta_base + 16 < sub_seq_len) {
            float b = beta_tile[beta_base + 16];
            beta1 = make_float2(b, b);
        }
        if (beta_base + 32 < sub_seq_len) {
            float b = beta_tile[beta_base + 32];
            beta2 = make_float2(b, b);
        }
        if (beta_base + 48 < sub_seq_len) {
            float b = beta_tile[beta_base + 48];
            beta3 = make_float2(b, b);
        }
        ...
        // === COMPUTE: fused kg_intra + qkg_intra for shared rows (saves sK+sG reloads) ===
        {
            if constexpr (WG_IDX == 0) {
                ...
                setup_intra_fused<
                    ...
                    kg_offset,
                    qkg_offset>(
                    sG, sK, sQ, sKG_intra, sQKG_intra, 1, idx_in_warpgroup, sub_seq_len, gn3, gn1, beta1, 4, 0);
                ...
                setup_intra_fused<
                    ...
                    kg_offset,
                    qkg_offset>(
                    sG, sK, sQ, sKG_intra, sQKG_intra, 2, idx_in_warpgroup, sub_seq_len, gn3, gn1, beta2, 5, 1);
            }
        }
        ...
        {
            Tensor sKG_inter =
                make_tensor(make_smem_ptr(shared_plan->kg_all.inter[0].data()), SmemLayoutMatBTF32Tranposed<1>{});
            Tensor sQKG_inter =
                make_tensor(make_smem_ptr(shared_plan->qkg_all.inter[0].data()), SmemLayoutMatBTF32Tranposed<2>{});

            if constexpr (WG_IDX == 0) {
                float2 beta0 = make_float2(0.0f, 0.0f);
                if (beta_base < sub_seq_len) {
                    float b = beta_tile[beta_base];
                    beta0 = make_float2(b, b);
                }
                ...
                setup_inter_fused<
                    ...
                    decltype(sQKG_inter),
                    kg_offset,
                    qkg_offset>(sG, sK, sQ, sKG_inter, sQKG_inter, 0, idx_in_warpgroup, sub_seq_len, beta0, gn_half_0);
                setup_inter_fused<
                    decltype(sG),
                    decltype(sK),
                    decltype(sQ),
                    ...
                    decltype(sQKG_inter),
                    kg_offset,
                    qkg_offset>(sG, sK, sQ, sKG_inter, sQKG_inter, 3, idx_in_warpgroup, sub_seq_len, beta3, gn_half_3);
            } else {
                ...
                setup_inter_fused<
                    ...
                    decltype(sQKG_inter),
                    kg_offset,
                    qkg_offset>(sG, sK, sQ, sKG_inter, sQKG_inter, 1, idx_in_warpgroup, sub_seq_len, beta1, gn_half_1);
                setup_inter_fused<
                    decltype(sG),
                    decltype(sK),
                    decltype(sQ),
                    ...
                    decltype(sQKG_inter),
                    kg_offset,
                    qkg_offset>(sG, sK, sQ, sKG_inter, sQKG_inter, 2, idx_in_warpgroup, sub_seq_len, beta2, gn_half_2);
            }
        }
        ...
        if (k_idx == 0) {
            mask_At_tensor<WG_IDX * 64, 64>(
                shared_plan->dAqk[buf_idx_A].data(),
                shared_plan->dAkk[buf_idx_A].data(),
                idx_in_warpgroup,
                sub_seq_len,
                tmem_addr::dAqk_t);
            tcgen05_before_thread_sync();
        ...
        // === EPILOGUE: precompute intra + inter scales in one fused pass over sG ===
        // Both overlap with MMA kg phase; saves redundant sG reads after bar_dq_done.
        float scale[HALF_K];
        float inter_scale[HALF_K];
        {
            int row = idx_in_warpgroup % 64;
            int intra_ref = row / 16 * 16;
            int inter_ref = min(row / 16 * 16 + 8, sub_seq_len - 1);
            if (row >= 16) {
                for (int i = 0; i < HALF_K / 4; ++i) {
                    float4 bg = *reinterpret_cast<float4*>(&sG(row, K_OFF + i * 4));
                    float4 bg_inter_ref = *reinterpret_cast<float4*>(&sG(inter_ref, K_OFF + i * 4));
                    float4 bg_intra_ref = *reinterpret_cast<float4*>(&sG(intra_ref, K_OFF + i * 4));
                    // Inter scale: exp2f(g[row] - g[half_row]) — for combine_dq_inter
                    float2 idiff0 =
                        float2_sub(reinterpret_cast<float2*>(&bg)[0], reinterpret_cast<float2*>(&bg_inter_ref)[0]);
                    float2 idiff1 =
                        float2_sub(reinterpret_cast<float2*>(&bg)[1], reinterpret_cast<float2*>(&bg_inter_ref)[1]);
                    inter_scale[i * 4] = exp2f(idiff0.x);
                    inter_scale[i * 4 + 1] = exp2f(idiff0.y);
                    inter_scale[i * 4 + 2] = exp2f(idiff1.x);
                    inter_scale[i * 4 + 3] = exp2f(idiff1.y);

                    // Intra scale: exp2f(g[row] - g[sub_tile_base]) — for apply_dq_intra
                    float2 diff0 =
                        float2_sub(reinterpret_cast<float2*>(&bg)[0], reinterpret_cast<float2*>(&bg_intra_ref)[0]);
                    float2 diff1 =
                        float2_sub(reinterpret_cast<float2*>(&bg)[1], reinterpret_cast<float2*>(&bg_intra_ref)[1]);
        ...
        // === COMPUTE: qkg_intra (non-overlapping rows only) ===
        {
            if constexpr (WG_IDX == 0) {
                ...
                setup_qkg_intra<decltype(sG), decltype(sQ), decltype(sK), decltype(sQKG_intra), qkg_offset>(
                    sG, sQ, sK, sQKG_intra, 3, idx_in_warpgroup, sub_seq_len, beta3, gn1, 2);
            } else {
                float4 gn2 = *reinterpret_cast<float4*>(&sG(32, y));
                float4 gn3 = *reinterpret_cast<float4*>(&sG(48, y));
                setup_qkg_intra<decltype(sG), decltype(sQ), decltype(sK), decltype(sQKG_intra), qkg_offset>(
                    sG, sQ, sK, sQKG_intra, 2, idx_in_warpgroup, sub_seq_len, beta2, gn2, 3);
                setup_qkg_intra_2gn<decltype(sG), decltype(sQ), decltype(sK), decltype(sQKG_intra), qkg_offset>(
                    sG, sQ, sK, sQKG_intra, 3, idx_in_warpgroup, sub_seq_len, beta3, gn2, gn3, 4, 5);
            }
        }
        ...
        // [DEBUG-B] Check inter scale (precomputed)
        DEBUG_CHECK_NAN("B:inter_scale", inter_scale, HALF_K, idx_in_warpgroup, k_idx);

        epilogue_combine_dq_inter<HALF_K>(tmem_addr::dq2 + K_OFF + 256 * buf_idx_value, res, inter_scale);

        // [DEBUG-C] Check res after dq_inter combine
        DEBUG_CHECK_NAN("C:res_dq_combined", res, HALF_K, idx_in_warpgroup, k_idx);

        // === EPILOGUE: compute dkt scale (moved before dq/db output to reduce post-bar_dkt_done path) ===
        ...
        // === EPILOGUE: output dq / accumulate db ===
        {
            if (idx_in_warpgroup >= 64) {
                ...
                float beta_val = (local_idx < sub_seq_len) ? beta_tile[local_idx] : 0.0f;
        ...
        epilogue_process_dkt<HALF_K>(
            idx_in_warpgroup, tmem_addr::dkt + K_OFF + 256 * buf_idx_value, res_dkt, scale, sub_seq_len);
        ...
        Tensor sDKT_0 =
            make_tensor(make_smem_ptr(shared_plan->b_k_neg_exp[buf_idx_value].data()), SmemLayoutDKTExchange{});
        Tensor sDKT_1 = make_tensor(make_smem_ptr(shared_plan->b_k_exp[buf_idx_value].data()), SmemLayoutDKTExchange{});
    ...
    // === DEFERRED DB REDUCE: WG0 partial -> smem -> WG1 accumulates (once after all k iterations) ===
    if (idx_in_warpgroup >= 64) {
    ...
    // === DB OUTPUT: WG1 holds the fully-reduced db ===
    if (idx_in_warpgroup >= 64 && WG_IDX == 1 && local_idx < sub_seq_len) {
        reinterpret_cast<float*>(
            params.db_out_ptr)[(start_offset + tile_idx * T_TILE + local_idx) * params.h + head_idx] = db;
    }
...
__global__ void
__launch_bounds__(NUM_THREADS, 1, 1) kda_bwd_intra_sm100_kernel(
    __grid_constant__ const KDA_bwd_intra_params params, __grid_constant__ const TmaParams tma_params) {
        ...
        if (elect_one_sync()) {
            ...
            for (int i = 0; i < NUM_BUF_A; ++i) {
                ...
                cute::initialize_barrier(
                    shared_plan->bar_dA_mask_ready[i],
                    64);  // repurposed: beta load by Empty warps (2 warps = 64 threads)
            }
    ...
    if (role == WarpRole::ComputeEpilogue) {
            ...
            for (;;) {
                ...
                for (int k_idx = 0; k_idx < K_ITERATION; ++k_idx) {
                    ...
                    // Launder shared_plan pointer to prevent compiler from hoisting qkg addresses into kg phase
                    {
                        ...
                        Tensor tDQ_02 =
                            partition_fragment_C(tile_mma_dqk_mask02, make_shape(Int<T_TILE>{}, Int<K_TILE>{}));
                        Tensor tDQ_13 =
                            partition_fragment_C(tile_mma_dqk_mask02, make_shape(Int<T_TILE>{}, Int<K_TILE>{}));
                        ...
                        // kg_intra: 3 MMA calls
                        {
                            ...
                            Tensor sKG_1 = make_tensor(
                                make_smem_ptr(sp->kg_all.intra[0].data()), SmemLayoutMatBTF32Tranposed<1>{});
                            ...
                            Tensor sKG_2 = make_tensor(
                                make_smem_ptr(sp->kg_all.intra[1].data()), SmemLayoutMatBTF32Tranposed<2>{});
                            ...
                            Tensor sKG_3 = make_tensor(
                                make_smem_ptr(sp->kg_all.intra[3].data()), SmemLayoutMatBTF32Tranposed<3>{});
...

                        {
                            ...
                            Tensor sKG_0 = make_tensor(
                                make_smem_ptr(sp->kg_all.inter[0].data()), SmemLayoutMatBTF32Tranposed<1>{});
                            ...
                            Tensor sKG_1 = make_tensor(
                                make_smem_ptr(sp->kg_all.inter[1].data()), SmemLayoutMatBTF32Tranposed<1>{});
                            ...
                            Tensor sKG_2 = make_tensor(
                                make_smem_ptr(sp->kg_all.inter[2].data()), SmemLayoutMatBTF32Tranposed<1>{});
                            ...
                            Tensor sKG_3 = make_tensor(
                                make_smem_ptr(sp->kg_all.inter[3].data()), SmemLayoutMatBTF32Tranposed<1>{});
                    ...
                    // Fresh laundered pointer — compiler cannot CSE with kg phase addresses
                    {
                        ...
                        Tensor tDKT_02 =
                            partition_fragment_C(tile_mma_dqk_mask0, make_shape(Int<T_TILE>{}, Int<K_TILE>{}));
                        Tensor tDKT_13 =
                            partition_fragment_C(tile_mma_dqk_mask0, make_shape(Int<T_TILE>{}, Int<K_TILE>{}));
                        ...
                        // qkg_intra: 3 MMA calls
                        {
                            ...
                            Tensor sKG_1 = make_tensor(
                                make_smem_ptr(sp->qkg_all.intra[0].data()), SmemLayoutMatBTF32Tranposed<6>{});
                            ...
                            Tensor sKG_2 = make_tensor(
                                make_smem_ptr(sp->qkg_all.intra[3].data()), SmemLayoutMatBTF32Tranposed<4>{});
                            ...
                            Tensor sKG_3 = make_tensor(
                                make_smem_ptr(sp->qkg_all.intra[5].data()), SmemLayoutMatBTF32Tranposed<2>{});
                        ...
                        // qkg_inter: 4 MMA calls
                        {
                            ...
                            Tensor sKG_0 = make_tensor(
                                make_smem_ptr(sp->qkg_all.inter[0].data()), SmemLayoutMatBTF32Tranposed<2>{});
                            ...
                            Tensor sKG_1 = make_tensor(
                                make_smem_ptr(sp->qkg_all.inter[1].data()), SmemLayoutMatBTF32Tranposed<2>{});
                            ...
                            Tensor sKG_2 = make_tensor(
                                make_smem_ptr(sp->qkg_all.inter[2].data()), SmemLayoutMatBTF32Tranposed<2>{});
                            ...
                            Tensor sKG_3 = make_tensor(
                                make_smem_ptr(sp->qkg_all.inter[3].data()), SmemLayoutMatBTF32Tranposed<2>{});
        ...
        if (elect_one_sync()) {
            ...
            for (;;) {
                ...
                for (int k_idx = 0; k_idx < K_ITERATION; ++k_idx) {
                    ...
                    Tensor sQ = make_tensor(make_smem_ptr(shared_plan->q[buf_idx_value].data()), SmemLayoutInputBF16{});
                    Tensor sDQ =
                        make_tensor(make_smem_ptr(shared_plan->dq[buf_idx_value].data()), SmemLayoutInputFP32{});
                    Tensor sK = make_tensor(make_smem_ptr(shared_plan->k[buf_idx_value].data()), SmemLayoutInputBF16{});
                    Tensor sG = make_tensor(make_smem_ptr(shared_plan->g[buf_idx_value].data()), SmemLayoutInputFP32{});
                    Tensor sDK =
                        make_tensor(make_smem_ptr(shared_plan->dk[buf_idx_value].data()), SmemLayoutInputFP32{});
                    Tensor sDG =
                        make_tensor(make_smem_ptr(shared_plan->dg[buf_idx_value].data()), SmemLayoutInputFP32{});
                    ...
                    Tensor mDQ = domain_offset(
                        make_coord(token_offset, _0{}, _0{}), tma_params.tma_dq.get_tma_tensor(tma_params.shape_qkg));
                    Tensor mDK = domain_offset(
                        make_coord(token_offset, _0{}, _0{}), tma_params.tma_dk.get_tma_tensor(tma_params.shape_qkg));
                    Tensor mDG = domain_offset(
                        make_coord(token_offset, _0{}, _0{}), tma_params.tma_dg.get_tma_tensor(tma_params.shape_qkg));
                    ...
                    cute::set_barrier_transaction_bytes(
                        shared_plan->bar_load_kg_ready[buf_idx_value], tma_transaction_kg_bytes);
                    ...
                    cute::set_barrier_transaction_bytes(
                        shared_plan->bar_load_qb[buf_idx_value], tma_transaction_q_bytes);
                    ...
                    cute::set_barrier_transaction_bytes(
                        shared_plan->bar_load_dkg_ready[buf_idx_value], tma_transaction_dkg_bytes);
        ...
        for (;;) {
            ...
            if (empty_idx < T_TILE) {
                if (empty_idx < sub_seq_len) {
                    int beta_offset = (token_offset + tile_idx * T_TILE + empty_idx) * params.h + head_idx;
                    shared_plan->beta_smem[A_phase][empty_idx] =
                        params.beta_is_fp32
                            ? reinterpret_cast<float*>(params.beta_ptr)[beta_offset]
                            : __bfloat162float(reinterpret_cast<__nv_bfloat16*>(params.beta_ptr)[beta_offset]);
                } else {
                    shared_plan->beta_smem[A_phase][empty_idx] = 0.0f;
                }
            }
            fence_view_async_shared();
...
void
run_kda_bwd_intra_sm100(KDA_bwd_intra_params& params, cudaStream_t stream) {
