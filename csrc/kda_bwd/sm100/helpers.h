...
using Layout_K_SW128_32B_Atom_Bits =
    ComposedLayout<Swizzle<2, 5, 2>, smem_ptr_flag, Layout<Shape<_4, _1024>, Stride<_1024, _1>>>;
...
CUTE_DEVICE
void
umma_arrive_noelect(uint64_t& bar_ptr) {
    uint64_t bar_intptr = cute::cast_smem_ptr_to_uint(&bar_ptr);
    asm volatile("tcgen05.commit.cta_group::1.mbarrier::arrive::one.shared::cluster.b64 [%0];" : : "l"(bar_intptr));
}
...
CUTE_DEVICE void
utcmma_ss(TiledMMA& tiled_mma, TensorA sA, TensorB sB, TensorFragC tC_frag, bool clear_accum) {
...
template <typename TiledMMA, typename TensorA, typename TensorB, typename TensorFragC>
CUTE_DEVICE void
utcmma_ts(TiledMMA& tiled_mma, TensorA tA_frag, TensorB sB, TensorFragC tC_frag, bool clear_accum) {
    tiled_mma.accumulate_ = clear_accum ? UMMA::ScaleOut::Zero : UMMA::ScaleOut::One;
    ThrMMA thr_mma = tiled_mma.get_slice(_0{});  // Since A/B/C are already CTA-local tiles, this number does not matter
    auto sB_frag = thr_mma.partition_fragment_B(sB);
...
CUTE_DEVICE void
tmem_ld_32dp32bNx(uint32_t const& src_addr, T* dst_ptr_) {
...
template <int N, typename T>
CUTE_DEVICE void
tmem_st_32dp32bNx(uint32_t const& dst_addr, T* src_ptr_) {
    static_assert(N > 0 && (N & (N - 1)) == 0 && N <= 128, "N must be a power of 2 and lies between 1 ~ 128");
    ...
    if constexpr (N == 1) {
        asm volatile(
            ...
            "[%1], {%0};\n"
            :
            : "r"(src_ptr[0]), "r"(dst_addr));
    } else if constexpr (N == 2) {
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x2.b32"
            "[%2], {%0, %1};\n"
            :
            : "r"(src_ptr[0]), "r"(src_ptr[1]), "r"(dst_addr));
    } else if constexpr (N == 4) {
        asm volatile(
