...
#include "kda_bwd/sm100/kda_bwd_common.cuh"
...
using WYSmemLayoutInputBF16_K = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW64_Atom<bf16>{}, Shape<Int<WY_T_TILE>, Int<WY_K_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// Layout for BF16 inputs [T_TILE, V_TILE]
using WYSmemLayoutInputBF16_V = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW64_Atom<bf16>{}, Shape<Int<WY_T_TILE>, Int<WY_V_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// Layout for FP32 inputs [T_TILE, K_TILE]
using WYSmemLayoutInputFP32_K = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW128_Atom<float>{}, Shape<Int<WY_T_TILE>, Int<WY_K_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// Layout for FP32 inputs [T_TILE, V_TILE]
using WYSmemLayoutInputFP32_V = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW128_Atom<float>{}, Shape<Int<WY_T_TILE>, Int<WY_V_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// Layout for A matrix [T_TILE, BT] - bf16 (Akk matrix)
using WYSmemLayoutA = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW64_Atom<bf16>{}, Shape<Int<WY_T_TILE>, Int<WY_T_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// Layout for H matrix [K_TILE, V_TILE]
using WYSmemLayoutH = decltype(coalesce(
    tile_to_shape(UMMA::Layout_K_SW128_Atom<float>{}, Shape<Int<WY_K_TILE>, Int<WY_V_TILE>>{}, Step<_1, _2>{}),
    Shape<_1, _1>{}));

// Layout for beta [T_TILE]
using WYSmemLayoutBeta = Layout<Shape<Int<WY_T_TILE>>, Stride<_1>>;
...
enum class WYWarpRole { Empty = 0x0, Load = 0x1, Mma = 0x2, Compute = 0x3, Epilogue = 0x4 };
...
__forceinline__ __device__ WYWarpRole
wy_warp_idx_to_role(int warp_idx) {
...
__global__ void
__launch_bounds__(WY_NUM_THREADS, 1, 1) kda_bwd_wy_dqkg_fused_sm100_kernel(
    __grid_constant__ const KDA_bwd_wy_dqkg_fused_params params, __grid_constant__ const TmaParams tma_params) {
...
void
run_kda_bwd_wy_dqkg_fused_sm100(KDA_bwd_wy_dqkg_fused_params& params, cudaStream_t stream) {
    ...
    auto tma_V = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((bf16*)params.v_ptr), make_layout(shape_V, stride_V)),
        WYSmemLayoutInputBF16_V{});

    auto tma_V_NEW = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((bf16*)params.v_new_ptr), make_layout(shape_V, stride_V)),
        WYSmemLayoutInputBF16_V{});
    ...
    auto tma_Beta = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((bf16*)params.beta_ptr), make_layout(shape_Beta, stride_Beta)),
        WYSmemLayoutBeta{});
    ...
    auto tma_H = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((float*)params.h_ptr), make_layout(shape_H, stride_H)),
        WYSmemLayoutH{});

    auto tma_DO = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((bf16*)params.do_ptr), make_layout(shape_V, stride_V)),
        WYSmemLayoutInputBF16_V{});

    auto tma_DH = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((float*)params.dh_ptr), make_layout(shape_H, stride_H)),
        WYSmemLayoutH{});

    auto tma_DV = cute::make_tma_copy(
        SM90_TMA_LOAD{},
        make_tensor(make_gmem_ptr((bf16*)params.dv_ptr), make_layout(shape_V, stride_V)),
        WYSmemLayoutInputBF16_V{});
