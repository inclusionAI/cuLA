...
// setup_kg_intra: compute exp2f(gn - g[j]) * k[j] for one (tile_j, gn) pair
template <typename G_TENSOR, typename K_TENSOR, typename KG_TENSOR, int KG_OFFSET>
__forceinline__ __device__ void
setup_kg_intra(
    G_TENSOR& sG, K_TENSOR& sK, KG_TENSOR& sKG_intra, int tile_j, int idx_in_warpgroup, float4& gn, int index) {
...
// setup_kg_inter: compute exp/neg_exp and neg_exp*k for one sub_tile
template <typename G_TENSOR, typename K_TENSOR, typename KG_TENSOR, typename EXP_TENSOR, int KG_OFFSET>
__forceinline__ __device__ void
setup_kg_inter(
    G_TENSOR& sG,
    K_TENSOR& sK,
    ...
    if (x < sub_seq_len) {
        ...
        reinterpret_cast<float2*>(&res_k)[0] =
            float2_mul(reinterpret_cast<float2*>(&res_neg_exp)[0], __bfloat1622float2(tmp_k.a));
        reinterpret_cast<float2*>(&res_k)[1] =
            float2_mul(reinterpret_cast<float2*>(&res_neg_exp)[1], __bfloat1622float2(tmp_k.b));
...
// setup_qkg_intra: already implemented
...
// setup_kg_intra_2gn: compute 2 kg_intra outputs from 1 sK+sG load (shared row, 2 different gn)
template <typename G_TENSOR, typename K_TENSOR, typename KG_TENSOR, int KG_OFFSET>
__forceinline__ __device__ void
setup_kg_intra_2gn(
    G_TENSOR& sG,
...
// setup_intra_fused: fused kg_intra + qkg_intra for same row (shared sK+sG+sQ load)
// Saves 2 smem loads (sK + sG) per call vs separate kg_intra + qkg_intra
template <
    typename G_TENSOR,
    typename K_TENSOR,
    typename Q_TENSOR,
...
// setup_qkg_intra_2gn: compute 2 qkg_intra outputs from 1 sK+sG+sQ load (shared row, 2 different gn)
template <typename G_TENSOR, typename Q_TENSOR, typename K_TENSOR, typename QKG_TENSOR, int QKG_OFFSET>
__forceinline__ __device__ void
setup_qkg_intra_2gn(
    G_TENSOR& sG,
    ...
    if (x < sub_seq_len) {
        ...
        s1b.y = exp2f(s1b.y);
        float4 rq1;
        reinterpret_cast<float2*>(&rq1)[0] = float2_mul(s1a, qf_a);
        reinterpret_cast<float2*>(&rq1)[1] = float2_mul(s1b, qf_b);
        ...
        s2b.y = exp2f(s2b.y);
        float4 rq2;
        reinterpret_cast<float2*>(&rq2)[0] = float2_mul(s2a, qf_a);
        reinterpret_cast<float2*>(&rq2)[1] = float2_mul(s2b, qf_b);
...
// setup_inter_fused: fused kg_inter + qkg_inter for same sub_tile
// Computes exp once, produces kg_inter + qkg_inter outputs
// No sBkExp/sBkNegExp writes — Epilogue computes directly from sG
template <
    typename G_TENSOR,
    typename K_TENSOR,
    typename Q_TENSOR,
...
setup_inter_fused(
    ...
    if (x < sub_seq_len) {
        ...
        float4 res_kg;
        reinterpret_cast<float2*>(&res_kg)[0] =
            float2_mul(reinterpret_cast<float2*>(&res_neg_exp)[0], __bfloat1622float2(tmp_k.a));
        reinterpret_cast<float2*>(&res_kg)[1] =
            float2_mul(reinterpret_cast<float2*>(&res_neg_exp)[1], __bfloat1622float2(tmp_k.b));
        store_128b(&sKG_inter(y, idx_in_warpgroup / 8) + KG_OFFSET * sub_tile_i, res_kg);

        // qkg_inter output: exp * q (q part)
        float4 res_q;
        reinterpret_cast<float2*>(&res_q)[0] =
            float2_mul(__bfloat1622float2(tmp_q.a), reinterpret_cast<float2*>(&res_exp)[0]);
        reinterpret_cast<float2*>(&res_q)[1] =
            float2_mul(__bfloat1622float2(tmp_q.b), reinterpret_cast<float2*>(&res_exp)[1]);
        store_128b(&sQKG_inter(y, idx_in_warpgroup / 8) + QKG_OFFSET * sub_tile_i, res_q);

        // qkg_inter output: exp * k * beta (k part)
        float4 res_kbeta;
        reinterpret_cast<float2*>(&res_kbeta)[0] =
            float2_mul(__bfloat1622float2(tmp_k.a), reinterpret_cast<float2*>(&res_exp)[0]);
        reinterpret_cast<float2*>(&res_kbeta)[1] =
            float2_mul(__bfloat1622float2(tmp_k.b), reinterpret_cast<float2*>(&res_exp)[1]);
...
__forceinline__ __device__ void
epilogue_compute_intra_scale(G_TENSOR& sG, int idx_in_warpgroup, float* scale) {
...
template <int K_TILE>
__forceinline__ __device__ void
epilogue_apply_dq_intra(int idx_in_warpgroup, int tmem_dq_addr, float* res, float* scale) {
    ...
    if (idx_in_warpgroup % 64 >= 16) {
        for (int i = 0; i < K_TILE / 2; ++i) {
            reinterpret_cast<float2*>(res)[i] =
                float2_mul(reinterpret_cast<float2*>(res)[i], reinterpret_cast<float2*>(scale)[i]);
        }
    } else {
        for (int i = 0; i < K_TILE / 2; ++i) {
...
template <int K_TILE>
__forceinline__ __device__ void
epilogue_combine_dq_inter(int tmem_dq2_addr, float* res, float* scale) {
    tcgen05_after_thread_sync();
    float res2[K_TILE];
...
__forceinline__ __device__ void
epilogue_output_dq(
    Q_TENSOR& sQ, DQ_TENSOR& sDQ, int idx_in_warpgroup, int sub_seq_len, float* res, __nv_bfloat16* dq_out_base) {
            ...
            for (int j = 0; j < 4; ++j) {
                ...
                for (int k = 0; k < 2; ++k) {
                    reinterpret_cast<float2*>(&tmp_dq)[k] = float2_add(
                        reinterpret_cast<float2*>(&tmp_dq)[k], reinterpret_cast<float2*>(&res[i * 16])[j * 2 + k]);
...
epilogue_accumulate_db(
    ...
    float2 beta2 = make_float2(beta_val, beta_val);
    for (int i = 0; i < K_TILE / 2; ++i) {
...
template <int K_TILE, int K_OFFSET = 0, typename G_TENSOR>
__forceinline__ __device__ void
epilogue_compute_dkt_scale(G_TENSOR& sG, int idx_in_warpgroup, int sub_seq_len, float* scale) {
    int row = idx_in_warpgroup % 64;
    bool should_zero = (idx_in_warpgroup < 64) ? ((row / 16 + 1) * 16 >= sub_seq_len) : (row >= sub_seq_len);
    if (should_zero) {
        for (int i = 0; i < K_TILE / 2; ++i) {
            reinterpret_cast<float2*>(scale)[i] = {0.0f, 0.0f};
        }
        return;
    }
    ...
    int ref_row =
        (idx_in_warpgroup < 64) ? min((row / 16 + 1) * 16, sub_seq_len - 1) : min(row / 16 * 16 + 8, sub_seq_len - 1);
    for (int i = 0; i < K_TILE / 4; ++i) {
...
template <int K_TILE>
__forceinline__ __device__ void
epilogue_process_dkt(int idx_in_warpgroup, int tmem_dkt_addr, float* res_dkt, float* scale, int sub_seq_len) {
    ...
    if (should_zero) {
        ...
        for (int i = 0; i < K_TILE / 2; ++i) {
            reinterpret_cast<float2*>(res_dkt)[i] =
                float2_mul(reinterpret_cast<float2*>(res_dkt)[i], reinterpret_cast<float2*>(scale)[i]);
        }
    }
}
...
__forceinline__ __device__ void
epilogue_exchange_dkt(DKT_TENSOR& sDKT_0, DKT_TENSOR& sDKT_1, int idx_in_warpgroup, float* res, float* res_dkt) {
    if (idx_in_warpgroup < 64) {
        ...
        for (int i = 0; i < K_TILE / 4; ++i) {
            float4 dk_sub_dkt;
            reinterpret_cast<float2*>(&dk_sub_dkt)[0] =
                float2_sub(reinterpret_cast<float2*>(res)[i * 2], reinterpret_cast<float2*>(res_dkt)[i * 2]);
            reinterpret_cast<float2*>(&dk_sub_dkt)[1] =
                float2_sub(reinterpret_cast<float2*>(res)[i * 2 + 1], reinterpret_cast<float2*>(res_dkt)[i * 2 + 1]);
...
epilogue_output_dg(
    ...
    for (int i = 0; i < K_TILE / 4; ++i) {
        ...
        float2 diff1 =
            float2_sub(reinterpret_cast<float2*>(&dk_sub_dkt)[1], reinterpret_cast<float2*>(res_dkt)[i * 2 + 1]);
    ...
    if (row < sub_seq_len) {
        for (int i = 0; i < K_TILE / 16; ++i) {
            ...
            for (int j = 0; j < 4; ++j) {
                ...
                for (int k = 0; k < 2; ++k) {
                    float2 sum = float2_add(
                        reinterpret_cast<float2*>(res_dkt + base)[k], reinterpret_cast<float2*>(&dkt_sub)[k]);
                    sum = float2_add(sum, reinterpret_cast<float2*>(&dk_input)[k]);
                    reinterpret_cast<float2*>(res + base)[k] =
                        float2_add(reinterpret_cast<float2*>(res + base)[k], sum);
