...
struct KdaChunkFwdRecompWUMainloopSm100 {
    ...
    // Akk (bf16)
    using SmemLayoutInputAkkBF16 = decltype(coalesce(tile_to_shape(
        UMMA::Layout_K_SW128_Atom<bf16>{},
    ...
    using SmemLayoutMatBBF16 = decltype(coalesce(tile_to_shape(
        UMMA::Layout_MN_SW128_Atom<bf16>{},
        Shape<Int<TileK>, Int<TileT>>{},
        Step<_1, _2>{}
    ), Shape<_1, _1>{}));

    // W/U/KG output (bf16)
    using SmemLayoutOutputBF16 = decltype(coalesce(tile_to_shape(
        UMMA::Layout_K_SW128_Atom<bf16>{},
        Shape<Int<TileT>, Int<TileK>>{},
        Step<_1, _2>{}
    ), Shape<_1, _1>{}));

    using TiledMMA_KDAak = decltype(
        make_tiled_mma(
    ...
    using GmemTiledCopyO =
        decltype(make_tiled_copy(GmemTileCopyAtomO{}, GmemLayoutAtom{}, Layout<Shape<_1, Int<kGmemElemsPerStore>>>{})); // Val layout, 8 vals per store
    ...
    CUTLASS_DEVICE void prologue_loop(
        ...
        PipelineKG &kg_pipeline, PipelineStateKG &kg_pipe_state_read,
        PipelineV &v_pipeline, PipelineStateV &v_pipe_state_read,
        ...
        PipelinePrologueReady &prologue_ready_pipeline, PipelineStatePrologueReady &prologue_ready_pipe_state_write
    )
    {
        ...
        for (; tile_scheduler.is_valid(); tile_scheduler.advance()) {
            ...
            for (int i_k = 0; i_k < NumKIters; ++i_k) {
                ...
                Tensor sK = make_tensor(make_smem_ptr(shared_plan->k[kg_pipe_state_read.index()].data()), SmemLayoutInputBF16{});
                Tensor sG = make_tensor(make_smem_ptr(shared_plan->g[kg_pipe_state_read.index()].data()), SmemLayoutInputFP32{});
                ...
                Tensor sV = make_tensor(make_smem_ptr(shared_plan->v[v_pipe_state_read.index()].data()), SmemLayoutInputBF16{});
                ...
                // Tensor sK_dst_kmajor = make_tensor(make_smem_ptr(shared_plan->k_mma[buf_idx].data()), SmemLayoutInputBF16{});
                // =====DEBUG=======
                // cutlass::arch::NamedBarrier::arrive_and_wait(NumPrologueThreads, KdaChunkFwdRecompWUSm100NamedBarriers::PrologueCudaCore);
                // if (idx_in_wg == 0) {
                //     printf("sK_dst\n");
    ...
    // ===================================================================
    template <typename TmaParamsT>
    CUTLASS_DEVICE void epilogue_loop(
        const KDA_fwd_recomp_w_u_params &params,
        const TmaParamsT &tma_params,
        SharedMemoryPlan *shared_plan,
        TileScheduler &tile_scheduler,
        ...
        PipelineAccDone &acc_done_pipeline, PipelineStateAccDone &acc_done_pipe_state_read
    )
    {
        ...
        for (; tile_scheduler.is_valid(); tile_scheduler.advance()) {
            ...
            Tensor mKg = make_tensor(
                ...
                make_layout(params.shape_wukg, params.stride_wukg)
            )(_, _, head_idx);
            ...
            for (int i_k = 0; i_k < NumKIters; ++i_k) {
                ...
                cutlass::arch::NamedBarrier::arrive_and_wait(NumEpilogueThreads, KdaChunkFwdRecompWUSm100NamedBarriers::EpilogueCudaCore);
                {
                    ...
                    cute::transform(tKrK, tKrG, tKrK, [&](auto k, auto g) {
                        return Element(float(k) * g);
                    });
                ...
                Tensor tOcO = gmem_thr_copy_O.partition_D(
                    make_identity_tensor(Shape<Int<TileT>, Int<TileK>>{}));
                Tensor tOpO = make_tensor<bool>(make_shape(size<2>(tOcO)));
                #pragma unroll
                for (int k = 0; k < size(tOpO); ++k) {
                    tOpO(k) = get<1>(tOcO(_0{}, _0{}, k)) < params.d;
                }
                ku::copy_pred</*Is_even_MN=*/false, /*Is_even_K=*/false,
                            /*Clear_OOB_MN=*/false, /*Clear_OOB_K=*/false>(
                    gmem_tiled_copy_O, tOsO, tOgKg, tOcO, tOpO, sub_seq_len);
                ...
                __nv_bfloat16 *out_ptr_base = reinterpret_cast<__nv_bfloat16 *>(
                    acc_idx == 0 ? params.w_out_ptr : params.u_out_ptr
                );
                ...
                __nv_bfloat16 *out_row_base = out_ptr_base
                    + (token_offset_cur + row) * params.d * params.h
                    + head_idx * params.d;
    ...
    CUTLASS_DEVICE void mma_loop(
        ...
        PipelineA &a_pipeline, PipelineStateA &a_pipe_state_read,
        // Prologue -> MMA pipeline (consumer, used for both K and V sequentially)
        PipelinePrologueReady &prologue_ready_pipeline, PipelineStatePrologueReady &prologue_ready_pipe_state_read,
        // MMA -> Epilogue pipeline (producer, used for both W and U sequentially)
        PipelineAccDone &acc_done_pipeline, PipelineStateAccDone &acc_done_pipe_state_write
    )
    {
        ...
        for (; tile_scheduler.is_valid(); tile_scheduler.advance()) {
            ...
            for (int i_k = 0; i_k < NumKIters; ++i_k) {
                ...
                int buf_mma_idx = acc_done_pipe_state_write.index();
                {
                    ...
                    Tensor sKmma = make_tensor(make_smem_ptr(shared_plan->k_mma[buf_mma_idx].data()), SmemLayoutMatBBF16{});
...

                {
                    ...
                    Tensor sVmma = make_tensor(make_smem_ptr(shared_plan->v_mma[buf_mma_idx].data()), SmemLayoutMatBBF16{});
    ...
    CUTLASS_DEVICE void load_loop(
        ...
        PipelineA &a_pipeline, PipelineStateA &a_pipe_state_write,
        PipelineKG &kg_pipeline, PipelineStateKG &kg_pipe_state_write,
        PipelineV &v_pipeline, PipelineStateV &v_pipe_state_write
    )
    {
        if (cute::elect_one_sync()) {
            ...
            for (; tile_scheduler.is_valid(); tile_scheduler.advance()) {
                ...
                Tensor mK = domain_offset(make_coord(token_offset, _0{}, _0{}), tma_params.tma_k.get_tma_tensor(tma_params.shape_kvg));
                Tensor mV = domain_offset(make_coord(token_offset, _0{}, _0{}), tma_params.tma_v.get_tma_tensor(tma_params.shape_kvg));
                Tensor mG = domain_offset(make_coord(token_offset, _0{}, _0{}), tma_params.tma_g.get_tma_tensor(tma_params.shape_kvg));
                Tensor mA = domain_offset(make_coord(token_offset, _0{}, _0{}), tma_params.tma_akk.get_tma_tensor(tma_params.shape_Akk));

                // ============================================================
                // Once per WU: TMA Akk[BT, BT] → sA
                // ============================================================
                {
                    Tensor sA = make_tensor(make_smem_ptr(shared_plan->akk[a_pipe_state_write.index()].data()), SmemLayoutInputAkkBF16{});
                    Tensor gA = local_tile(mA(_, _, head_idx), make_shape(Int<TileT>{}, Int<TileT>{}), make_coord(tile_idx, _0{}));
                    a_pipeline.producer_acquire(a_pipe_state_write);
                    ku::launch_tma_copy(tma_params.tma_akk, gA, sA, *a_pipeline.producer_get_barrier(a_pipe_state_write));
                    ++a_pipe_state_write;
                }
                ...
                CUTE_NO_UNROLL
                for (int i_k = 0; i_k < NumKIters; ++i_k) {
                    Tensor sK = make_tensor(make_smem_ptr(shared_plan->k[kg_pipe_state_write.index()].data()), SmemLayoutInputBF16{});
                    Tensor sV = make_tensor(make_smem_ptr(shared_plan->v[v_pipe_state_write.index()].data()), SmemLayoutInputBF16{});
                    Tensor sG = make_tensor(make_smem_ptr(shared_plan->g[kg_pipe_state_write.index()].data()), SmemLayoutInputFP32{});

                    Tensor gK = local_tile(mK(_, _, head_idx), make_shape(Int<TileT>{}, Int<TileK>{}), make_coord(tile_idx, i_k));
                    Tensor gV = local_tile(mV(_, _, head_idx), make_shape(Int<TileT>{}, Int<TileK>{}), make_coord(tile_idx, i_k));
                    Tensor gG = local_tile(mG(_, _, head_idx), make_shape(Int<TileT>{}, Int<TileK>{}), make_coord(tile_idx, i_k));
    ...
    // ===================================================================
    template <typename TmaParamsT>
    CUTLASS_DEVICE void load_aux_loop(
        const KDA_fwd_recomp_w_u_params &params,
        const TmaParamsT &tma_params,
        SharedMemoryPlan *shared_plan,
        TileScheduler &tile_scheduler,
        // Beta pipeline (producer, 1×/WU)
        PipelineBeta &beta_pipeline, PipelineStateBeta &beta_pipe_state_write
    )
    {
        ...
        for (; tile_scheduler.is_valid(); tile_scheduler.advance()) {
            ...
            if (thread_idx < TileT) {
                float beta_val = (thread_idx < sub_seq_len)
                    ? reinterpret_cast<float*>(params.beta_ptr)[(token_offset + tile_idx * TileT + thread_idx) * params.h + head_idx]
                    : float(0);
