...
template <typename ArchTag,  // TODO: hide this
          ...
          typename TState>
void
launch_kda_fwd_prefill_kernel(cudaStream_t stream,
                              TO* output,
                              TState* output_state,
                              TQKV const* q,
                              TQKV const* k,
...
#define LAUNCH(needs_beta, needs_alpha, init_state, safe_gate)                                                   \
    launch_kda_fwd_prefill_kernel_gbai<needs_beta, needs_alpha, init_state, safe_gate, ArchTag>(                 \
        ...
        num_heads, head_size, total_seqlen, scale, sm_count);
    if (init_state) {
...
template void
launch_kda_fwd_prefill_kernel<cutlass::arch::Sm90, bf16, bf16, float>(cudaStream_t stream,
                                                                      bf16* output,
                                                                      float* state,
                                                                      bf16 const* q,
                                                                      bf16 const* k,
