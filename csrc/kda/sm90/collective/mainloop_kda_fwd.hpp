...
struct FlatMainloopTmaWarpSpecializedKdaFwd {
  ...
  using TiledMmaQK_RS = decltype(
    make_tiled_mma(
      decltype(cute::GMMA::rs_op_selector<Element, Element, ElementAccumulatorQK, TileShapeQK>()){},
      AtomLayoutQK{}
    )
  );
  ...
  using SmemLayoutAlpha_SD = decltype(
    tile_to_shape(
      SmemLayoutAlphaAtom{},
      make_shape(shape<1>(TileShapeQK{}), shape<2>(TileShapeQK{}), Int<StagesAlpha::value>{})
    )
  ); // (blk_kv, head_size), (64, 128)
  ...
  using SmemLayoutQ_K_Scaled_SD = decltype(unstage_smem_layout(typename CollectiveMmaQK::SmemLayoutA{}, Int<StagesQ_K_Scaled::value>{}));
  using SmemLayoutK_DS = decltype(unstage_smem_layout(typename CollectiveMmaKV_G2S::SmemLayoutB{}, Int<StagesK::value>{}));
  using SmemLayoutQ_K_Scaled_DS = decltype(unstage_smem_layout(typename CollectiveMmaKV_G2S::SmemLayoutB{}, Int<StagesQ_K_Scaled::value>{}));
  ...
  using CollectiveMmaO1 = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, RefLayoutKV, Alignment, // NOTE: S (KV) as operand A
      Element, LayoutQ, Alignment,
      ElementAccumulatorO,
  ...
  using CollectiveMmaO2 = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, RefLayoutV, Alignment, // V^T
  ...
  struct SharedStorage {
    ...
    alignas(alignment_for_swizzle(QKScaledSmemLayoutQ{})) cute::array_aligned<Element, cute::cosize_v<QKScaledSmemLayoutQ>> smem_q_k_scaled;
  ...
  load_qkv(
      ...
      MainloopAlphaPipeline& alpha_pipeline, AlphaPipelineState& alpha_smem_pipe_write,
  ...
  extract_alpha_last(
      ...
      MainloopAlphaLastPipeline& alpha_last_pipeline, AlphaLastPipelineState& alpha_last_smem_pipe_write,
      SharedStorage& storage
  ) {
  ...
  compute(
      ...
      MainloopAlphaLastPipeline& alpha_last_pipeline, AlphaLastPipelineState& alpha_last_smem_pipe_read,
  ...
  compute_aux_safe(
    ...
    auto qk_kk_subchunk_mma_and_store = [&](int blk) INLINE_LAMBDA {
      ...
      // returns (tQKrQ, tQKrK, tArAfirst_kt) = (Q * exp2(g - g_first), K * exp2(g - g_first), g_first in B layout)
      auto s2r_compute_subchunk_operandA = [&](auto r_, int j, int j0, int j1) INLINE_LAMBDA {
        ...
        cute::transform(tQKrQ_r_j_bf16, tArA_r_j, tQKrQ_r_j_float, [&](auto q, auto g) {
          return float(q) * g;
        });
        ...
        cute::transform(tQKrK_r_j_bf16, tArA_r_j, tQKrK_r_j_float, [&](auto k, auto g) {
          return float(k) * g;
        });
        // convert BF16 MMA layout → TF32 MMA layout in-place via warp shuffles
      ...
      // tArAfirst_kt: pre-loaded g_first register tensor (BF16 MMA B layout) for computing gktn = exp2(g_first - g_c)
      // returns tQKrKt = K_c * exp2(g_first - g_c)
      auto s2r_compute_subchunk_operandB = [&](auto c_, int j, int j0, int j1, auto const& tArAfirst_kt) INLINE_LAMBDA {
        ...
        cute::transform(tArA_c_j, tArAfirst_kt, tArA_c_j, [&](auto g, auto g_first) {
          return exp2f(g_first - g);
        });
        ...
        cute::transform(tQKrKt_c_j_bf16, tArA_c_j, tQKrKt_c_j_float, [&](auto k, auto g) {
          return float(k) * g;
        });
      ...
      auto gemm_tensor_core_1x16x16x128 = [&](auto r_, auto c_, auto is_diagonal_, auto is_first_subchunk_) INLINE_LAMBDA {
