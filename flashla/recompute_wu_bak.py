...
class KDARecomputeWU:
    ...
    def __call__(
        ...
        # v: non-MMA epilog TMA (separate SMEM buffer)
        ...
        self.tma_bytes_kgk = self.tma_bytes_k + self.tma_bytes_gk
    ...
    def kernel(
        ...
        bSG_sK, bSG_gK = self._data_tma_partition(
            tma_atom_k, tma_tensor_k, (self.BT, self.BK), sK, i_h, i_b,
        )

        # v: non-MMA → (BT, BV) tiles (separate SMEM)
        bSG_sV, bSG_gV = self._data_tma_partition(
            tma_atom_v, tma_tensor_v, (self.BT, self.BV), sV, i_h, i_b,
        )
        ...
        # =================================================================
            ...
            for i_k in cutlass.range(0, num_k_tiles, unroll=0):
                ...
                # Release SMEM immediately — done reading sK/sGK
                kgk_h.release()
            ...
            for i_v in cutlass.range(0, num_v_tiles, unroll=0):
                ...
                for ei in cutlass.range_constexpr(cute.size(tTR_cM)):
                    m_coord, k_coord = tTR_cM[ei]
                    v_val = sV[(k_coord, m_coord, v_h.index)].to(self.acc_dtype)
                    beta_val = sBeta[k_coord].to(self.acc_dtype)
