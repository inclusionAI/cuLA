...
"""
SM100 training KDA fused K1+K2+K3 intra kernel, fp32-Akk fused-inverse variant.

This module is intentionally separate from
`cula/ops/flash_infer/kda_fuse_k123.py`.  It keeps the FlashInfer K123 pipeline
shape, but changes K2 to the training precision boundary: fp32 gated register
operands, TF32 register MMA, and fp32 accumulators.

Fuses gate activation + cumsum + scaling (K1), intra sub-chunk Aqk/Akk (K2),
and K3 stores into a single persistent kernel.

This variant keeps pre-inverse Akk in fp32 SMEM and runs the TF32 inverse inside
the K3 store warpgroup.  The raw-gate `sG` TMA buffer and fp32 `sAkk` use two
views over the same physical shared-memory allocation. G is kept compact in the
row dimension because CuTeDSL/TMA misaligns with the fully padded row-stride-136
shared layout:

  sG_raw_bf16: [64,128,2], stride=(128,1,64*68*2)
  sAkk_fp32:   [64, 68,2], stride=( 68,1,64*68)

This is the first bring-up step toward CUDA-C style fused inverse. K2 writes
`sAkk` in logical lower-triangular layout for the fp32 TF32 inverse helper, not
the older block-transposed pre-inverse GMEM scratch layout.
...
SMEM: ~189KB payload (q+k × [64,128] bf16 × 2 stages
      + g/sAkk alias [64,68,2] fp32 + g_cumsum [64,136] fp32 × 2 stages
      + sPartialPrefix [8,132] fp32 + sAqk [16,168,2] bf16
      + barriers/beta)
...
Outputs (g_cumsum stays in SMEM, not written to GMEM):
  ...
  A_kk       [B,T,H,BT]  bf16  final lower_tri((I + L)^-1), no beta[col] epilogue
"""
...
MMA_K_TILE = 16
NUM_MMA_K_TILES = K_DIM // MMA_K_TILE  # 8, loaded with BF16 m16n8k16 fragment layout
AQK_TILE_PAD = 8
AQK_TILE_STRIDE = BT + AQK_TILE_PAD  # 72 — sAqk now 64x72 row-major (same shape as sAkk)

AKK_PAD = 4
AKK_STRIDE = BT + AKK_PAD  # 68, fp32 padded stride matching CUDA-C SmemLayoutInvKK_TF32_Padded
G_ALIAS_ROW_STRIDE_BF16 = K_DIM  # compact G row; TMA padded row stride 136 causes misaligned address
G_ALIAS_STAGE_STRIDE_BF16 = BT * AKK_STRIDE * 2  # physical fp32 sAkk stage pitch, in bf16 elements
AKK_TMP_STRIDE = BC + 4
AKK_TMP_SLOTS = 4
...
@dsl_user_op
def fma_rn_f32(a, b, c, *, loc=None, ip=None):
    """PTX fma.rn.f32, used to mirror Triton's cumsum lowering."""
...
@cute.kernel
def kda_fwd_intra_sm100_training_triton_reduce_kernel(
    ...
    # One physical buffer, two lifetime-disjoint views:
    #   - sG:   bf16 raw gate for TMA/K1, compact row stride 128
    #   - sAkk: fp32 pre-inverse Akk for K2/K3 and future in-kernel inverse, row stride 68
    ...
    sG = cute.make_tensor(cute.recast_ptr(sAkk.iterator, dtype=cutlass.BFloat16), g_smem_layout)
    akk_tmp_layout = cute.make_layout(
        (NUM_STAGES, AKK_TMP_SLOTS, BC, BC),
        stride=(AKK_TMP_SLOTS * BC * AKK_TMP_STRIDE, BC * AKK_TMP_STRIDE, AKK_TMP_STRIDE, 1),
    )
    sAkkTmp = smem.allocate_tensor(cutlass.Float32, akk_tmp_layout, 128)
    sGcum = smem.allocate_tensor(cutlass.Float32, g_cumsum_layout, 128)
    partial_prefix_layout = cute.make_layout((K1_ROW_GROUPS, PARTIAL_COLS), stride=(PARTIAL_COLS, 1))
    sPartialPrefix = smem.allocate_tensor(cutlass.Float32, partial_prefix_layout, 128)
    ...
    aqk_tile_layout = cute.make_layout((BT, AQK_TILE_STRIDE, NUM_STAGES), stride=(AQK_TILE_STRIDE, 1, BT * AQK_TILE_STRIDE))
    sAqk = smem.allocate_tensor(cutlass.BFloat16, aqk_tile_layout, 128)
    # sAkk is fp32 in this variant. K3 normalizes it to I+strict-lower(L),
    # runs the TF32 inverse in-place, then stores the final bf16 Akk.
    ...
    for _s in cutlass.range_constexpr(NUM_STAGES):
        for _ri in cutlass.range_constexpr(2):
            ...
            sAkk[_row, _col_lo, _s] = cutlass.Float32(0.0)
            sAkk[_row, _col_hi, _s] = cutlass.Float32(0.0)
    cute.arch.barrier()
    ...
    for work_id in for_generate(block_id, total_cgs, NUM_SMS):
        ...
        if warp_idx < NUM_K1_TMA_WARPS:
            ...
            for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
                ...
                # Triton 3.5.1 lowering for
                # tl.cumsum([BT=64, BS=64], axis=0): 4-row local scan,
                # then left-fold 4-row group sums through shared memory.
                # Recompute the gate here so the first local scan add can
                # use fma.rn.f32, as ptxas does for lower_bound * sigmoid + prev.
                for half in cutlass.range_constexpr(2):
                    row0 = warp_row_group * 4 + half * 32
                    row1 = row0 + 1
                    row2 = row0 + 2
                    row3 = row0 + 3

                    sG_tile0 = cute.local_tile(csG, tiler=(1, K1_COLS_PER_WARP), coord=(row0, warp_col_group))
                    tCsG0 = thr_copy_k1.partition_S(sG_tile0)
                    tCrG0 = cute.make_fragment_like(tCsG0)
                    cute.copy(tiled_copy_qk_k1, tCsG0, thr_copy_k1.retile(tCrG0))

                    sG_tile1 = cute.local_tile(csG, tiler=(1, K1_COLS_PER_WARP), coord=(row1, warp_col_group))
                    tCsG1 = thr_copy_k1.partition_S(sG_tile1)
                    tCrG1 = cute.make_fragment_like(tCsG1)
                    cute.copy(tiled_copy_qk_k1, tCsG1, thr_copy_k1.retile(tCrG1))

                    sG_tile2 = cute.local_tile(csG, tiler=(1, K1_COLS_PER_WARP), coord=(row2, warp_col_group))
                    tCsG2 = thr_copy_k1.partition_S(sG_tile2)
                    tCrG2 = cute.make_fragment_like(tCsG2)
                    cute.copy(tiled_copy_qk_k1, tCsG2, thr_copy_k1.retile(tCrG2))

                    sG_tile3 = cute.local_tile(csG, tiler=(1, K1_COLS_PER_WARP), coord=(row3, warp_col_group))
                    tCsG3 = thr_copy_k1.partition_S(sG_tile3)
                    tCrG3 = cute.make_fragment_like(tCsG3)
                    cute.copy(tiled_copy_qk_k1, tCsG3, thr_copy_k1.retile(tCrG3))

                    for vi in cutlass.range_constexpr(VEC):
                        c = col_base + vi

                        g_val0 = tCrG0[vi].to(cutlass.Float32)
                        g_val1 = tCrG1[vi].to(cutlass.Float32)
                        g_val2 = tCrG2[vi].to(cutlass.Float32)
                        g_val3 = tCrG3[vi].to(cutlass.Float32)
                        if HAS_BIAS:
                            g_val0 = g_val0 + rBias[vi]
                            g_val1 = g_val1 + rBias[vi]
                            g_val2 = g_val2 + rBias[vi]
                            g_val3 = g_val3 + rBias[vi]

                        gate_scale = cutlass.Float32(0.0)
                        gate_arg0 = cutlass.Float32(0.0)
                        gate_arg1 = cutlass.Float32(0.0)
                        gate_arg2 = cutlass.Float32(0.0)
                        gate_arg3 = cutlass.Float32(0.0)
                        if USE_SAFE_GATE:
                            gate_scale = lower_bound
                            gate_arg0 = fast_rcp(cutlass.Float32(1.0) + cute.exp2(-exp_A * g_val0 * LOG2E, fastmath=True))
                            gate_arg1 = fast_rcp(cutlass.Float32(1.0) + cute.exp2(-exp_A * g_val1 * LOG2E, fastmath=True))
                            gate_arg2 = fast_rcp(cutlass.Float32(1.0) + cute.exp2(-exp_A * g_val2 * LOG2E, fastmath=True))
                            gate_arg3 = fast_rcp(cutlass.Float32(1.0) + cute.exp2(-exp_A * g_val3 * LOG2E, fastmath=True))
                        else:
                            gate_scale = -exp_A
                            gate_arg0 = (
                                cute.log2(cutlass.Float32(1.0) + cute.exp2(g_val0 * LOG2E, fastmath=True), fastmath=True) * LN2
                            )
                            gate_arg1 = (
                                cute.log2(cutlass.Float32(1.0) + cute.exp2(g_val1 * LOG2E, fastmath=True), fastmath=True) * LN2
                            )
                            gate_arg2 = (
                                cute.log2(cutlass.Float32(1.0) + cute.exp2(g_val2 * LOG2E, fastmath=True), fastmath=True) * LN2
                            )
                            gate_arg3 = (
                                cute.log2(cutlass.Float32(1.0) + cute.exp2(g_val3 * LOG2E, fastmath=True), fastmath=True) * LN2
                            )

                        if IS_VARLEN and not VARLEN_PURE:
                            if chunk_start + row0 >= ci_eos:
                                gate_arg0 = cutlass.Float32(0.0)
                            if chunk_start + row1 >= ci_eos:
                                gate_arg1 = cutlass.Float32(0.0)
                            if chunk_start + row2 >= ci_eos:
                                gate_arg2 = cutlass.Float32(0.0)
                            if chunk_start + row3 >= ci_eos:
                                gate_arg3 = cutlass.Float32(0.0)

                        gate0 = gate_scale * gate_arg0
                        gate1 = gate_scale * gate_arg1
                        gate2 = gate_scale * gate_arg2

                        scan0 = gate0
                        scan1 = fma_rn_f32(gate_scale, gate_arg1, gate0)
                        scan2_stage1 = fma_rn_f32(gate_scale, gate_arg2, gate1)
                        scan3_stage1 = fma_rn_f32(gate_scale, gate_arg3, gate2)
                        scan2 = scan2_stage1 + gate0
                        scan3 = scan3_stage1 + scan1
                ...
                for vi in cutlass.range_constexpr(VEC):
                    c = col_base + vi
                    sPartialPrefix[warp_row_group, c] = csGcum[warp_row_group * 4 + 3, c]
                ...
                prefix_lane_base = col_in_group * K1_ROW_GROUPS

                for j in cutlass.range_constexpr(PARTIAL_COLS_PER_WARP // 4):
                    col = prefix_col_start + j * 4 + col_in_group

                    group0 = cutlass.Float32(sPartialPrefix[row_in_prefix, col])
                    running0 = cutlass.Float32(0.0)
                    inclusive0 = cutlass.Float32(0.0)
                    for src_group in cutlass.range_constexpr(K1_ROW_GROUPS):
                        src_val0 = cute.arch.shuffle_sync(group0, prefix_lane_base + src_group)
                        running0 = running0 + src_val0
                        if row_in_prefix == src_group:
                            inclusive0 = running0

                    sPartialPrefix[row_in_prefix, col] = inclusive0
                ...
                for vi in cutlass.range_constexpr(VEC):
                    ...
                    if warp_row_group > 0:
                        rPrefix[vi] = sPartialPrefix[warp_row_group - 1, c]
                    rGkLast[vi] = sPartialPrefix[K1_ROW_GROUPS - 1, c]

                k1_internal_barrier()

                for vi in cutlass.range_constexpr(VEC):
                    c = col_base + vi
                    sPartialPrefix[warp_row_group, c] = csGcum[32 + warp_row_group * 4 + 3, c]

                k1_internal_barrier()

                for j in cutlass.range_constexpr(PARTIAL_COLS_PER_WARP // 4):
                    col = prefix_col_start + j * 4 + col_in_group
                    prefix_local_col = warp_row_group * PARTIAL_COLS_PER_WARP + j * 4 + col_in_group
                    total0_src_lane = prefix_local_col // VEC
                    total0_v0 = cute.arch.shuffle_sync(rGkLast[0], total0_src_lane)
                    total0_v1 = cute.arch.shuffle_sync(rGkLast[1], total0_src_lane)
                    total0 = total0_v0
                    if prefix_local_col % VEC != 0:
                        total0 = total0_v1

                    group1 = cutlass.Float32(sPartialPrefix[row_in_prefix, col])
                    running1 = total0
                    prefix1 = total0
                    for src_group in cutlass.range_constexpr(K1_ROW_GROUPS):
                        if row_in_prefix == src_group:
                            prefix1 = running1
                        src_val1 = cute.arch.shuffle_sync(group1, prefix_lane_base + src_group)
                        running1 = running1 + src_val1

                    sPartialPrefix[row_in_prefix, col] = prefix1

                k1_internal_barrier()

                for vi in cutlass.range_constexpr(VEC):
                    c = col_base + vi
                    rGkLast[vi] = sPartialPrefix[warp_row_group, c]

                for vi in cutlass.range_constexpr(VEC):
                    for half in cutlass.range_constexpr(2):
                        row0 = warp_row_group * 4 + half * 32
                        prefix_prev = rPrefix[vi]
                        if half == 1:
                            prefix_prev = rGkLast[vi]
                        for rr in cutlass.range_constexpr(4):
                            row = row0 + rr
                            t = chunk_start + row
                            c = col_base + vi
                            scan_val = csGcum[row, c] + prefix_prev
                            g_cum_val = scan_val * cumsum_scale
                            csGcum[row, c] = g_cum_val
                k1_internal_barrier()
                # Signal MMA early: csGcum is ready
                cute.arch.mbarrier_arrive(k1_done_mbars + cur_stage)

                # ---- Pass 2b: write K/Q/KG GMEM (overlaps with MMA, off critical path) ----
                for ri in cutlass.range_constexpr(ROWS_PER_K1_WARP):
                    ...
                    for vi in cutlass.range_constexpr(VEC):
                        ...
                        last_row = BT - 1
                        if IS_VARLEN and not VARLEN_PURE:
                            last_row = ci_eos - chunk_start - 1
                        gk_last_cs = csGcum[last_row, col_base + vi]
                ...
                if warp_row_group == 0:
                    for vi in cutlass.range_constexpr(VEC):
                        ...
                        if IS_VARLEN and not VARLEN_PURE:
                            last_row = ci_eos - chunk_start - 1
                        gk_last_out = csGcum[last_row, col_base + vi]
                        rGkOut[vi] = cute.exp2(gk_last_out, fastmath=True)
                    if IS_VARLEN:
        ...
        if warp_idx == TMA_WARP_ID:
            ...
            ts_pf, tg_pf = cpasync.tma_partition(
                tma_atom_Q, 0, cute.make_layout(1), cute.group_modes(sQ_pf, 0, 2), cute.group_modes(gQ_pf, 0, 2)
            )
            ...
            # sG aliases fp32 sAkk.  Unlike sQ/sK, it cannot be reused at the
            # early MMA-read-done point; wait until K3/store has released the
            # stage so TMA G cannot overwrite an in-flight fp32 Akk tile.
            ...
            for next_i in cutlass.range_constexpr(1, CHUNKS_PER_BLOCK):
                ...
                ts_ns, tg_ns = cpasync.tma_partition(
                    tma_atom_Q, 0, cute.make_layout(1), cute.group_modes(sQ_ns, 0, 2), cute.group_modes(gQ_ns, 0, 2)
                )
                ...
                # sG aliases sAkk; G TMA waits for K3/store completion for
                # this stage, while Q/K can still use the early stage_reuse.
        ...
        if warp_idx >= NUM_K1_TMA_WARPS and warp_idx < NUM_K1_TMA_WARPS + NUM_MMA_WARPS and warp_idx != TMA_WARP_ID:
            ...
            akk_row_base = q_row_base
            akk_col_base = k_row_base
            norm_row = q_row_base
            if my_i_q == my_i_k:
            ...
            for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
                ...
                if mma_warp < NUM_MMA_ACTIVE:
                    ...
                    # Each k_block loads the existing BF16 m16n8k16 fragment
                    # layout, forms gated fp32 register operands, then issues
                    # two TF32 m16n8k8 MMAs per n-tile.
                    for k_block in cutlass.range_constexpr(NUM_MMA_K_TILES):
                        ...
                        # TF32 m16n8k8 consumes one k8 half per call.  The
                        # loaded BF16-fragment value order is (r0,k0),
                        # (r0,k1), (r8,k0), (r8,k1), so reorder A to the
                        # register MMA order used by kda_fuse_k4.py:
                        # (r0,k0), (r8,k0), (r0,k1), (r8,k1).
                        acc_aqk_n0_0, acc_aqk_n0_1, acc_aqk_n0_2, acc_aqk_n0_3 = mma_tf32_m16n8k8(
                            qa0, qa2, qa1, qa3, k_n0_b0, k_n0_b1, acc_aqk_n0_0, acc_aqk_n0_1, acc_aqk_n0_2, acc_aqk_n0_3
                        )
                        acc_aqk_n0_0, acc_aqk_n0_1, acc_aqk_n0_2, acc_aqk_n0_3 = mma_tf32_m16n8k8(
                            qa4, qa6, qa5, qa7, k_n0_b2, k_n0_b3, acc_aqk_n0_0, acc_aqk_n0_1, acc_aqk_n0_2, acc_aqk_n0_3
                        )

                        acc_aqk_n1_0, acc_aqk_n1_1, acc_aqk_n1_2, acc_aqk_n1_3 = mma_tf32_m16n8k8(
                            qa0, qa2, qa1, qa3, k_n1_b0, k_n1_b1, acc_aqk_n1_0, acc_aqk_n1_1, acc_aqk_n1_2, acc_aqk_n1_3
                        )
                        acc_aqk_n1_0, acc_aqk_n1_1, acc_aqk_n1_2, acc_aqk_n1_3 = mma_tf32_m16n8k8(
                            qa4, qa6, qa5, qa7, k_n1_b2, k_n1_b3, acc_aqk_n1_0, acc_aqk_n1_1, acc_aqk_n1_2, acc_aqk_n1_3
                        )

                        acc_akk_n0_0, acc_akk_n0_1, acc_akk_n0_2, acc_akk_n0_3 = mma_tf32_m16n8k8(
                            ka0, ka2, ka1, ka3, k_n0_b0, k_n0_b1, acc_akk_n0_0, acc_akk_n0_1, acc_akk_n0_2, acc_akk_n0_3
                        )
                        acc_akk_n0_0, acc_akk_n0_1, acc_akk_n0_2, acc_akk_n0_3 = mma_tf32_m16n8k8(
                            ka4, ka6, ka5, ka7, k_n0_b2, k_n0_b3, acc_akk_n0_0, acc_akk_n0_1, acc_akk_n0_2, acc_akk_n0_3
                        )

                        acc_akk_n1_0, acc_akk_n1_1, acc_akk_n1_2, acc_akk_n1_3 = mma_tf32_m16n8k8(
                            ka0, ka2, ka1, ka3, k_n1_b0, k_n1_b1, acc_akk_n1_0, acc_akk_n1_1, acc_akk_n1_2, acc_akk_n1_3
                        )
                        acc_akk_n1_0, acc_akk_n1_1, acc_akk_n1_2, acc_akk_n1_3 = mma_tf32_m16n8k8(
                            ka4, ka6, ka5, ka7, k_n1_b2, k_n1_b3, acc_akk_n1_0, acc_akk_n1_1, acc_akk_n1_2, acc_akk_n1_3
                        )
                    ...
                    _z32 = cutlass.Float32(0.0)
                    _one32 = cutlass.Float32(1.0)
                    if IS_VARLEN and not VARLEN_PURE:
                        if my_i_q == my_i_k:
        ...
        if warp_idx >= NUM_K1_TMA_WARPS + NUM_MMA_WARPS:
            ...
            for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
                ...
                # K2 writes sAkk in logical tile layout, but the pure equal-len
                # path writes diagonal 16x16 tiles without causal masking. Make
                # the inverse input exactly I + strict-lower(L) in fp32 SMEM.
                ...
                for norm_i in cutlass.range_constexpr((BT * BT) // (NUM_STORE_WARPS * 32)):
                    ...
                    norm_row = linear // BT
                    norm_col = linear % BT
                    norm_val = cutlass.Float32(csAkk[norm_row, norm_col])
                ...
                csTmp = sAkkTmp[(s, None, None, None)]
                ...
                if store_tid < 64:
                    ...
                    c0, c1, c2, c3, c4, c5, c6, c7 = _tf32_matmul16_smem_smem(
                        csAkk, row_base + 16, row_base + 16, csAkk, row_base + 16, row_base, lane_id
                    )
                    _tf32_store_C16_tmp(csTmp, block32, -c0, -c1, -c2, -c3, -c4, -c5, -c6, -c7, lane_id)
                store_internal_barrier()
                ...
                if store_tid < 64:
                    ...
                    c0, c1, c2, c3, c4, c5, c6, c7 = _tf32_matmul16_tmp_smem(
                        csTmp, block32, csAkk, row_base, row_base, lane_id
                    )
                    _tf32_store_C16_smem(csAkk, row_base + 16, row_base, c0, c1, c2, c3, c4, c5, c6, c7, lane_id)
                store_internal_barrier()

                # 32x32 -> 64x64. Four store warps compute partials over x and
                # reduce by y, matching the standalone fp32 TF32 inverse.
                inv_x = store_warp // 2
                inv_y = store_warp % 2
                inv_slot = store_warp
                ...
                p0, p1, p2, p3, p4, p5, p6, p7 = _tf32_matmul16_smem_smem(csAkk, inv_row_o, 32, csAkk, 32, inv_col_c, lane_id)
                q0, q1, q2, q3, q4, q5, q6, q7 = _tf32_matmul16_smem_smem(csAkk, inv_row_o, 48, csAkk, 48, inv_col_c, lane_id)
                _tf32_store_C16_tmp(
                    csTmp,
                    inv_slot,
                    -(p0 + q0),
                ...
                o0, o1, o2, o3, o4, o5, o6, o7 = _tf32_matmul16_tmp_smem(csTmp, inv_slot, csAkk, inv_x * 16, 0, lane_id)
                r0, r1, r2, r3, r4, r5, r6, r7 = _tf32_matmul16_tmp_smem(csTmp, inv_slot, csAkk, inv_x * 16, 16, lane_id)
                if inv_x == 0:
                    _tf32_store_C16_smem(csAkk, inv_row_o, 0, o0, o1, o2, o3, o4, o5, o6, o7, lane_id)
                    _tf32_store_C16_smem(csAkk, inv_row_o, 16, r0, r1, r2, r3, r4, r5, r6, r7, lane_id)
                store_internal_barrier()
                if inv_x == 1:
                    _tf32_add_store_C16_smem(csAkk, inv_row_o, 0, o0, o1, o2, o3, o4, o5, o6, o7, lane_id)
                    _tf32_add_store_C16_smem(csAkk, inv_row_o, 16, r0, r1, r2, r3, r4, r5, r6, r7, lane_id)
                store_internal_barrier()

                # Final Akk output is CUDA-C/FLA no-beta logical lower-tri.
                rAkkFinal = cute.make_rmem_tensor(cute.make_layout((VEC,)), cutlass.BFloat16)
                row_base_warp = store_warp * (BT // NUM_STORE_WARPS)
                for ri in cutlass.range_constexpr(BT // NUM_STORE_WARPS):
                    local_row = row_base_warp + ri
                    abs_row = chunk_start + local_row
                    ...
                    rAkkFinal[0] = val0.to(cutlass.BFloat16)
                    rAkkFinal[1] = val1.to(cutlass.BFloat16)
                    if IS_VARLEN and not VARLEN_PURE:
                        if abs_row < st_eos:
                            cute.autovec_copy(rAkkFinal, mAkk_v2[i_b, abs_row, i_h, col_vec_idx, None])
                    else:
                        cute.autovec_copy(rAkkFinal, mAkk_v2[i_b, abs_row, i_h, col_vec_idx, None])
...
def make_host_function(
    ...
    def host_fn(
        ...
        smem_atom_qk = tcgen05.make_smem_layout_atom(tcgen05.SmemLayoutAtomKind.K_SW128, cutlass.BFloat16)
        ...
        # Raw G shares storage with fp32 sAkk in this variant.  The logical G
        # TMA tile is compact per row (stride 128) because padded row stride
        # 136 trips a TMA misaligned-address failure.  The stage stride still
        # follows the larger fp32 sAkk physical stage pitch, so stage 1 starts
        # at the correct alias offset.
        g_smem_2d = cute.make_layout((BT, K_DIM), stride=(G_ALIAS_ROW_STRIDE_BF16, 1))
        g_smem_3d = cute.make_layout((BT, K_DIM, NUM_STAGES), stride=(G_ALIAS_ROW_STRIDE_BF16, 1, G_ALIAS_STAGE_STRIDE_BF16))
        ...
        tiled_copy_mma_A = cute.make_tiled_copy_A(
            cute.make_copy_atom(cute.nvgpu.warp.LdMatrix8x8x16bOp(False, 4), cutlass.BFloat16), tiled_mma_k2
        )
        tiled_copy_mma_B = cute.make_tiled_copy_B(
            cute.make_copy_atom(cute.nvgpu.warp.LdMatrix8x8x16bOp(False, 2), cutlass.BFloat16), tiled_mma_k2
        )

        copy_atom_Gcum = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), cutlass.Float32, num_bits_per_copy=64)
        ...
        smem_size = (
            ...
            + BT * AKK_STRIDE * 4 * NUM_STAGES  # aliased sG bf16 padded / sAkk fp32
            + NUM_STAGES * AKK_TMP_SLOTS * BC * AKK_TMP_STRIDE * 4  # sAkk inverse tmp
        ...
        kda_fwd_intra_sm100_training_triton_reduce_kernel(
            tma_atom_Q,
            tma_tensor_Q,
            tma_atom_K,
            tma_tensor_K,
...
def _ct_cached(t, etype):
    """Build and cache a CuTe tensor wrapper for a PyTorch tensor."""
    key = (id(t), t._version, etype)
    entry = _input_wrap_cache.get(key)
    if entry is None:
        entry = from_dlpack(t, assumed_align=16).mark_layout_dynamic()
        entry.element_type = etype
        _input_wrap_cache[key] = entry
    return entry


def _get_eqlen_dummies(device):
    """Dummy varlen tensors for equal-length launches; kernel does not read them."""
    key = device.index if device.index is not None else 0
    entry = _eqlen_dummy_cache.get(key)
    if entry is None:
        cu = torch.empty(2, dtype=torch.int64, device=device)
        ci = torch.empty(1, 2, dtype=torch.int64, device=device)
        cu_ct = from_dlpack(cu, assumed_align=4).mark_layout_dynamic()
        ci_ct = from_dlpack(ci, assumed_align=4).mark_layout_dynamic()
        cu_ct.element_type = cutlass.Int64
        ci_ct.element_type = cutlass.Int64
        entry = (cu, ci, cu_ct, ci_ct)
        _eqlen_dummy_cache[key] = entry
    return entry[2], entry[3]


def _get_empty_bias_ct(device):
    key = device.index if device.index is not None else 0
    entry = _empty_bias_cache.get(key)
    if entry is None:
        bias = torch.empty(1, 1, dtype=torch.float32, device=device)
        bias_ct = from_dlpack(bias, assumed_align=16).mark_layout_dynamic(1)
        bias_ct.element_type = cutlass.Float32
        entry = (bias, bias_ct)
        _empty_bias_cache[key] = entry
    return entry[1]


def _get_dt_bias_ct(dt_bias, H, K):
    key = (id(dt_bias), dt_bias._version, H, K)
    entry = _dt_bias_cache.get(key)
    if entry is None:
        bias = dt_bias.contiguous().view(H, K)
        bias_ct = from_dlpack(bias, assumed_align=16).mark_layout_dynamic()
        bias_ct.element_type = cutlass.Float32
        entry = (bias, bias_ct)
...
def chunk_kda_fwd_intra_sm100_training_triton_reduce(
    ...
    *,
    ...
    """Launch the SM100 training K1+K2+K3 intra kernel with in-CTA Akk inverse.

    This variant always uses the Triton-style K1 row tree/reduce scan that
    matches FLA `kda_gate_chunk_cumsum` bit-exactly. It returns K123
    intermediates; `A_kk` is final no-beta inverse output, not pre-inverse
    scratch.
    """
    if chunk_size != BT:
        raise NotImplementedError(f"chunk_size must be {BT}, got {chunk_size}.")
    if q.dim() != 4 or k.dim() != 4 or g.dim() != 4 or beta.dim() != 3:
        raise ValueError("Expected q/k/g [B,T,H,K] and beta [B,T,H].")
    if q.shape != k.shape or q.shape != g.shape:
        raise ValueError(f"q/k/g shapes must match, got q={q.shape}, k={k.shape}, g={g.shape}.")
    B, T_total, H, K = q.shape
    if K != K_DIM:
        raise NotImplementedError(f"SM100 training intra currently supports K={K_DIM}, got {K}.")
    if T_total % (CHUNKS_PER_BLOCK * BT) != 0:
        raise NotImplementedError(
            f"equal-length launch requires T to be a multiple of {CHUNKS_PER_BLOCK * BT}; got T={T_total}."
        )
    if beta.shape != (B, T_total, H):
        raise ValueError(f"beta shape must be {(B, T_total, H)}, got {tuple(beta.shape)}.")
    if A_log.shape != (H,):
        raise ValueError(f"A_log shape must be {(H,)}, got {tuple(A_log.shape)}.")
    if q.dtype != torch.bfloat16 or k.dtype != torch.bfloat16 or g.dtype != torch.bfloat16:
        raise NotImplementedError("q/k/g must be torch.bfloat16 for the current K123 TMA layout.")
    if beta.dtype != torch.bfloat16:
        raise NotImplementedError("beta must be torch.bfloat16 for this first training wrapper.")
    if A_log.dtype != torch.float32:
        raise NotImplementedError("A_log must be torch.float32.")
    if dt_bias is not None and dt_bias.dtype != torch.float32:
        raise NotImplementedError("dt_bias must be torch.float32 when provided.")
    if not (q.is_cuda and k.is_cuda and g.is_cuda and beta.is_cuda and A_log.is_cuda):
        raise ValueError("q/k/g/beta/A_log must all be CUDA tensors.")
    if dt_bias is not None and not dt_bias.is_cuda:
        raise ValueError("dt_bias must be a CUDA tensor when provided.")
    if not (q.device == k.device == g.device == beta.device == A_log.device):
        raise ValueError("q/k/g/beta/A_log must be on the same CUDA device.")
    if dt_bias is not None and dt_bias.device != q.device:
        raise ValueError("dt_bias must be on the same CUDA device as q.")
    if not (q.is_contiguous() and k.is_contiguous() and g.is_contiguous() and beta.is_contiguous() and A_log.is_contiguous()):
        raise ValueError("q/k/g/beta/A_log must be contiguous for this first wrapper.")
    if dt_bias is not None and dt_bias.numel() != H * K_DIM:
        raise ValueError(f"dt_bias must contain H*K={H * K_DIM} values, got {dt_bias.numel()}.")
    if scale is None:
        scale = K_DIM**-0.5
    ...
    alog_ct = _ct_cached(A_log, cutlass.Float32)
    ...
    akk_ct = _ct_cached(A_kk, cutlass.BFloat16)
    cu_ct, ci_ct = _get_eqlen_dummies(q.device)
    bias_ct = _get_dt_bias_ct(dt_bias, H, K_DIM) if has_bias else _get_empty_bias_ct(q.device)
    ...
    kernel = _k123_training_cache.get(cache_key)
    if kernel is None:
        ...
        _k123_training_cache[cache_key] = kernel
    kernel(*ct_args)

    result = (k_scaled, kg, q_scaled, gk_last_exp, A_qk, A_kk)
    return result


def chunk_kda_fwd_intra_sm100_training_triton_reduce_with_akk_inv(
    q: torch.Tensor,
    k: torch.Tensor,
    g: torch.Tensor,
    beta: torch.Tensor,
    ...
    *,
    ...
    """Launch K123 and return the already fused FLA/CUDA-C chunk-forward `Akk`.

    This fused variant computes `Akk = (I + L_beta)^-1` inside the K3
    store/inverse warpgroup, without the final beta[col] epilogue.
    """
    return chunk_kda_fwd_intra_sm100_training_triton_reduce(
        q=q,
        k=k,
        g=g,
        beta=beta,
...
def chunk_kda_fwd_intra_sm100_training_triton_reduce_with_akk_inv_beta_epilogue(
    q: torch.Tensor,
    k: torch.Tensor,
    g: torch.Tensor,
    beta: torch.Tensor,
    ...
    *,
    ...
    """Beta[col] epilogue is not wired for the fused-inverse bring-up variant."""
    raise NotImplementedError("fused-inverse beta[col] epilogue is not implemented yet")
