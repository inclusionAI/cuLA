...
def _compile_occ(H, K, V, chunk_size=64, beta_dtype=cutlass.Float32):
    key = (H, K, V, chunk_size, beta_dtype)
    if key in _occ_cache:
        return _occ_cache[key]

    kernel_obj = KDARecomputeWUOcc(K=K, V=V, chunk_size=chunk_size, beta_dtype=beta_dtype)
...
def recompute_w_u_fwd_occ(k, v, beta, A, gk):
    ...
    compiled_fn = _compile_occ(H, K, V, chunk_size=BT, beta_dtype=beta_dtype)
