...
from fla.ops.kda.chunk_bwd import (
    recompute_w_u_fwd as recompute_w_u_fwd_triton,
)
...
# SM90 backward recompute still needs the forward-H computation; route it to FLA
# Triton directly instead of through a cula.ops shell module.
...
@functools.cache
def _get_chunk_delta_h_fwd_for_sm(major: int, minor: int):
    if major == 10 and minor in (0, 3):
        return importlib.import_module("cula.ops.chunk_delta_h_sm100").chunk_gated_delta_rule_fwd_h
...
@functools.cache
def _get_cutedsl_wy_dqkg_for_sm(major: int, minor: int):
    if major == 10 and minor in (0, 3):
        return importlib.import_module("cula.ops.chunk_wy_dqkg_sm100").chunk_kda_bwd_wy_dqkg_fused
    if major == 9 and minor == 0:
...
def _get_chunk_delta_h_fwd(device: torch.device):
    return _get_chunk_delta_h_fwd_for_sm(*_get_device_sm_version(device))
...
_RECOMPUTE_W_U_TRITON = "triton"
_RECOMPUTE_W_U_CUDA_EXTENSION = "cuda_extension"


def _select_recompute_w_u_backend(device: torch.device):
    major, minor = _get_device_sm_version(device)
    if major == 9 and minor == 0:
        return _RECOMPUTE_W_U_TRITON
    if major == 10 and minor in (0, 3):
        return _RECOMPUTE_W_U_CUDA_EXTENSION
    raise RuntimeError(
        f"Unsupported CUDA compute capability sm_{major}{minor}. "
        f"Only sm90a (Hopper) and Blackwell (SM100/SM103) are supported."
    )
...
def _select_chunk_kda_bwd_wy_dqkg_fused(q: torch.Tensor, v: torch.Tensor):
    ...
    if major == 9 and minor == 0:
        if q.shape[2] == v.shape[2]:
            return _get_cutedsl_wy_dqkg_for_sm(major, minor)
...
def chunk_kda_bwd(
    ...
    **kwargs,
    ...
    if disable_recompute is False:
        ...
        recompute_w_u_backend = _select_recompute_w_u_backend(q.device)
        if cu_seqlens is not None and chunk_indices is None:
            chunk_indices = prepare_chunk_indices(cu_seqlens, chunk_size)
        reset_cu_seqlens = False
        if recompute_w_u_backend == _RECOMPUTE_W_U_CUDA_EXTENSION:
            if cu_seqlens is None:
                reset_cu_seqlens = True
                cu_seqlens = _prepare_uniform_cu_seqlens(B, T, q.device, torch.int32)
                chunk_indices = prepare_chunk_indices(cu_seqlens, chunk_size)
        ...
        chunk_gated_delta_rule_fwd_h = _get_chunk_delta_h_fwd(q.device)
