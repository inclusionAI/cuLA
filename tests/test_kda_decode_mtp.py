...
from cula.ops.kda_decode_mtp_kvbuffer import (
    ...
    kda_decode_mtp_gemm_kvbuffer_cute,
    kda_decode_mtp_kvbuffer,
    kda_decode_mtp_tp_kvbuffer,
    kda_flush_kvbuffer,
)
...
def test_ws_decode(N, T, H, HV, tile_v, ilp_rows, use_smem_v):
...
def test_small_batch_decode(N, T, H, HV, variant, bv, k_split):
    """small_batch vk + kv vs loop: bv / k_split / auto / GQA in one table."""
    K, V = 128, 128
    scale = K**-0.5
    q, k, v, a, b, A_log, dt_bias, state = make_inputs_mtp(N, T, H, HV, K, V)
    o_loop, st_loop = run_kda_decode_mtp_via_loop_dense(q, k, v, a, b, A_log, dt_bias, state, scale)
...
@pytest.mark.parametrize("kernel", ["ws", "ws_smem_v", "sb_vk", "sb_kv"])
...
def test_ws_ilp4_rejects_bad_tile_v():
...
def _alloc_ubufs(N, T, HV, V, device="cuda"):
    """u_buffer [N,T,HV,V], kinv/b_buffer [N,T,HV,K] — fp32, matching the kernel contract."""
    return (
...
def _check_kvb_verify_and_flush(which, N, T, H, HV):
    ...
    assert ubufs[0].abs().sum() > 0, f"{which}: u_buffer was not written"
...
def test_tp_kvbuffer_verify_and_flush(N, T, H, HV):
    """tp-kvbuffer (token-parallel SIMT) verify output + rank-m flush match the fp32 oracle."""
    _check_kvb_verify_and_flush("tp", N, T, H, HV)
...
def test_kvbuffer_dispatch_routes_by_T(T, routed):
...
@pytest.mark.parametrize("which,N,T,H,HV", [("tp", 4, 4, 16, 16), ("cg", 4, 6, 16, 16)])
...
def test_select_tp_kvb_ilp_rows_invariants(tile_v, T):
    """ilp_rows must divide rows_per_group = tile_v/4 (the wrapper asserts this)."""
    ilp = _select_tp_kvb_ilp_rows(tile_v, T)
