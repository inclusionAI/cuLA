...
def test_safe_gate_chunk(
    ...
    from cula.gdn.gate import naive_gdn_lowerbound_gate

    cula_gdn_fused_fwd = get_gdn_fused_fwd(device)

    torch.manual_seed(42)
    q = torch.rand(B, T, H, D, dtype=dtype)
    k = torch.rand(B, T, H, D, dtype=dtype)
    v = torch.rand(B, T, H, D, dtype=dtype)
    g = torch.randn(B, T, H, dtype=torch.float if not use_gate_in_kernel else dtype)
    if use_gate_in_kernel:
        A_log = torch.randn(H, dtype=torch.float)
        dt_bias = torch.randn(H, dtype=torch.float)
    else:
    ...
    if safe_gate:
        ...
        if not use_gate_in_kernel:
            g = g.clamp(-5, 0)
        naive_gdn_gate_fn = naive_gdn_lowerbound_gate
    else:
        lower_bound = None
        naive_gdn_gate_fn = naive_gdn_gate
    ...
    # Unlike KDA, GDN does not have fused gate preprocessing kernel. It only does chunk_local_cumsum, so the gate_fn must be applied outside
    ref_fla, ref_ht_fla = chunk_gated_delta_rule(
...
@pytest.mark.parametrize(
    ("H", "D", "mask_p", "cu_seqlens", "dtype", "safe_gate", "use_gate_in_kernel"),
    [
        pytest.param(*test, id="H{}-D{}-mask_p{}-cu_seqlens{}-{}-safe_gate{}-gate{}".format(*test))
        for test in [
            (4, 128, 0.1, [0, 15], torch.bfloat16, True, False),
            (4, 128, 0.9, [0, 256, 500, 1000], torch.bfloat16, True, False),
            (4, 128, 0.5, [0, 256, 500, 1000], torch.bfloat16, True, False),
            (4, 128, 0, [0, 15, 100, 300, 1200, 2000], torch.bfloat16, True, False),
            (4, 128, 0, [0, 100, 300, 1200, 3000, 4096], torch.bfloat16, True, False),
...
def test_safe_gate_chunk_varlen(
    ...
    g = torch.randn(1, T, H, dtype=torch.float if not use_gate_in_kernel else dtype)
    if use_gate_in_kernel:
        A_log = torch.randn(H, dtype=torch.float)
        dt_bias = torch.randn(H, dtype=torch.float)
    else:
